...
constexpr int C_TILES_PER_CTA = 64;
...
#endif
#ifndef HOPPER_GQA_WARP_COMPACT_WRITE
#define HOPPER_GQA_WARP_COMPACT_WRITE 0
#endif
...
#endif
// Warp-specialized register reallocation (sm_90 setmaxnreg). When enabled, the
// producer warpgroup deallocates registers it does not need (TMA + bookkeeping)
// and donates them to the two consumer (WGMMA) warpgroups, which hold the 32
// float accumulators per thread.
#ifndef HOPPER_SPARSE_SETMAXNREG
#define HOPPER_SPARSE_SETMAXNREG 0
#endif
#ifndef HOPPER_SPARSE_PRODUCER_REGS
#define HOPPER_SPARSE_PRODUCER_REGS 32
#endif
#ifndef HOPPER_SPARSE_CONSUMER_REGS
#define HOPPER_SPARSE_CONSUMER_REGS 232
#endif
...
#endif
// Candidate write-out strategy for the m64n8 full sparse kernel (bs<=8):
//   0 (default) = per-lane atomicAdd(&qcount[row], 1) then direct store.
//   1           = 8-row register queue (tidal-style fixed-mask 0x11/22/44/88 +
//                 (i&1) row parity), with the 8 replicated per-row counts folded
//                 into ONE warp-distributed register.  Batches candidates per row
//                 so the whole warp reserves buffer space with a SINGLE leader
//                 atomicAdd per flush, cutting qcount atomic contention.
// Measured on MSMARCO D=768: neutral at small k; the queue pulls ahead only at
// very large k (bs=8: ~-2% at k=8192, ~-7% at k=16384) where the m64n8 tile's
// 4x-per-row lane contention on atomicAdd becomes the visible cost.
...
#if HOPPER_SPARSE_SETMAXNREG
  if (warpgroup == 0) {
    warpgroup_reg_dealloc<HOPPER_SPARSE_PRODUCER_REGS>();
  } else {
    warpgroup_reg_alloc<HOPPER_SPARSE_CONSUMER_REGS>();
  }
#endif

#if HOPPER_THR_REFRESH_GROUP > 0
  // Overlap threshold refresh with the initial TMA flight. Consumers read the
  // latest visible th in the epilogue; a stale th is conservative, not wrong.
#if 0 && HOPPER_SPARSE_REFRESH_FROM_BUF == 1
  if (warpgroup == 0) {
    static_assert((BM % THR_REFRESH_GROUP) == 0,
                  "THR_REFRESH_GROUP must divide BM for producer refresh");
    constexpr int REFRESH_ROWS_PER_CTA = BM / THR_REFRESH_GROUP;
    constexpr int PRODUCER_WARPS = 4;
    int lane_id = tid & 31;
    int producer_warp = warp;  // warpgroup 0 owns warp ids [0, 3].
    int row_base = (blockIdx.y % THR_REFRESH_GROUP) * REFRESH_ROWS_PER_CTA;

    for (int row_slot = producer_warp; row_slot < REFRESH_ROWS_PER_CTA; row_slot += PRODUCER_WARPS) {
      int row_local = row_base + row_slot;
      int row = cta_m_base + row_local;
      int locked = 0;
      int start = 0;
      int scan_to = 0;
      if (lane_id == 0 && NB <= REFRESH_NB_MAX &&
          atomicCAS(&refresh_lock[row], 0, 1) == 0) {
        int end = qcount[row];
        if (end > BUF) end = BUF;
        start = qprev[row];
        if (start < 0) start = 0;
      ...
      locked = __shfl_sync(0xffffffffu, locked, 0);
      start = __shfl_sync(0xffffffffu, start, 0);
      scan_to = __shfl_sync(0xffffffffu, scan_to, 0);

      int32_t* hrow = refresh_hist + row_local * REFRESH_NB_MAX;
      if (locked) {
        for (int b = lane_id; b < REFRESH_NB_MAX; b += 32) {
          hrow[b] = 0;
        }
      }
      __syncwarp();

      for (int p = start + lane_id;; p += 32) {
        int active = locked && (p < scan_to);
        unsigned active_mask = __ballot_sync(0xffffffffu, active);
        if (active_mask == 0) break;
        if (active) {
          float v = __half2float(buf_val[(size_t)row * BUF + p]);
          int braw = int((v - origin[row]) * inv_delta[row]);
          int b = braw < 0 ? 0 : (braw > NB - 1 ? NB - 1 : braw);
          unsigned peers = __match_any_sync(active_mask, b);
          int leader = __ffs(peers) - 1;
          if (lane_id == leader) {
            atomicAdd(&hrow[b], __popc(peers));
          }
        }
      }
      __syncwarp();
      ...
      if (lane_id == 0 && locked) {
        qprev[row] = scan_to;
        CUTE_UNROLL
        for (int b = 0; b < REFRESH_NB_MAX; ++b) {
          if (b < NB && hrow[b] != 0) {
            atomicAdd(&bcount[(row * NB + b) * BCOUNT_SHARDS], hrow[b]);
          }
        }
        ...
        atomicMin(&th[row], new_th);
        __threadfence();
        atomicExch(&refresh_lock[row], 0);
      }
      __syncwarp();
    }
  }
#elif HOPPER_SPARSE_REFRESH_FROM_BUF > 0
  if (blockIdx.y > 0 && (blockIdx.y % THR_REFRESH_GROUP) == 0) {
#if HOPPER_RFB_FLATTEN_ONE_SYNC
    if (tid == 0) {
      int total = 0;
      refresh_prefix_s[0] = 0;
      refresh_done_s = 0;
      CUTE_UNROLL
      for (int row_local = 0; row_local < BM; ++row_local) {
        int row = cta_m_base + row_local;
        int locked = 0;
        int start = 0;
        int end = 0;
        if (NB <= REFRESH_NB_MAX && atomicCAS(&refresh_lock[row], 0, 1) == 0) {
          end = qcount[row];
          if (end > BUF) end = BUF;
          start = qprev[row];
          if (start < 0) start = 0;
          if (start > end) start = end;
          locked = 1;
#if HOPPER_RFB_EARLY_UNLOCK
          qprev[row] = end;
          __threadfence();
          atomicExch(&refresh_lock[row], 0);
#endif
        }
        refresh_start_s[row_local] = start;
        refresh_end_s[row_local] = end;
        refresh_locked_s[row_local] = locked;
        int len = locked ? (end - start) : 0;
        total += len;
        refresh_prefix_s[row_local + 1] = total;
      }
    }

    for (int i = tid; i < BM * REFRESH_NB_MAX; i += THREADS) {
      refresh_hist[i] = 0;
    }
    __syncthreads();

    int total_candidates = refresh_prefix_s[BM];
    for (int linear = tid; linear < total_candidates; linear += THREADS) {
      int lo = 0;
      int hi = BM;
      while (lo < hi) {
        int mid = (lo + hi) >> 1;
        if (refresh_prefix_s[mid + 1] <= linear) {
          lo = mid + 1;
        } else {
          hi = mid;
        }
      ...
      int row_local = lo;
      int row = cta_m_base + row_local;
      int p = refresh_start_s[row_local] + (linear - refresh_prefix_s[row_local]);
      int b = 0;
#if HOPPER_SPARSE_REFRESH_FROM_BUF == 2
      b = (int)buf_bucket[(size_t)row * BUF + p];
#else
      float v = __half2float(buf_val[(size_t)row * BUF + p]);
      int braw = int((v - origin[row]) * inv_delta[row]);
...
#endif
      if (b < NB) {
        atomicAdd(&refresh_hist[row_local * REFRESH_NB_MAX + b], 1);
      }
    }

#if HOPPER_RFB_FLATTEN_PARALLEL_FLUSH
    __syncthreads();
    if (tid < BM && refresh_locked_s[tid]) {
      int row = cta_m_base + tid;
      int32_t* hrow = refresh_hist + tid * REFRESH_NB_MAX;
      int gate = th[row];
#if !HOPPER_RFB_EARLY_UNLOCK
      qprev[row] = refresh_end_s[tid];
#endif
      CUTE_UNROLL
      for (int b = 0; b < REFRESH_NB_MAX; ++b) {
#if HOPPER_RFB_FLUSH_SKIP_ABOVE_GATE
        if (b < NB && b <= gate && hrow[b] != 0) {
#else
        if (b < NB && hrow[b] != 0) {
#endif
          atomicAdd(&bcount[(row * NB + b) * BCOUNT_SHARDS], hrow[b]);
...
#else
    __threadfence_block();
    int done = atomicAdd(&refresh_done_s, 1);
    if (done == THREADS - 1) {
      CUTE_UNROLL
      for (int row_local = 0; row_local < BM; ++row_local) {
        if (!refresh_locked_s[row_local]) continue;
        int row = cta_m_base + row_local;
        int32_t* hrow = refresh_hist + row_local * REFRESH_NB_MAX;
        int gate = th[row];
#if !HOPPER_RFB_EARLY_UNLOCK
        qprev[row] = refresh_end_s[row_local];
#endif
        CUTE_UNROLL
        for (int b = 0; b < REFRESH_NB_MAX; ++b) {
#if HOPPER_RFB_FLUSH_SKIP_ABOVE_GATE
...
#endif
#elif HOPPER_RFB_PRODUCER_ONLY
    if (warpgroup == 0) {
      int lane_id = tid & 31;
      int producer_warp = warp;  // warpgroup 0 owns warp ids [0, 3].
      for (int row_local = producer_warp; row_local < BM; row_local += 4) {
        int row = cta_m_base + row_local;
        int locked = 0;
        int start = 0;
        int scan_to = 0;
        ...
        if (lane_id == 0 && locked) {
          ...
          for (int b = 0; b < REFRESH_NB_MAX; ++b) {
            ...
            if (b < NB && hrow[b] != 0) {
#endif
#if HOPPER_RFB_GLOBAL_BATCH_ADD && !HOPPER_RFB_EARLY_UNLOCK
              bcount[(row * NB + b) * BCOUNT_SHARDS] += hrow[b];
#else
              atomicAdd(&bcount[(row * NB + b) * BCOUNT_SHARDS], hrow[b]);
#endif
            }
...
#elif HOPPER_RFB_CANDIDATE_PARALLEL
    if (tid < BM) {
      int row = cta_m_base + tid;
      int locked = 0;
      int start = 0;
      ...
      refresh_start_s[tid] = start;
      refresh_end_s[tid] = end;
      refresh_locked_s[tid] = locked;
    }
    __syncthreads();

    for (int i = tid; i < BM * REFRESH_NB_MAX; i += THREADS) {
      int row_local = i / REFRESH_NB_MAX;
      if (refresh_locked_s[row_local]) {
        refresh_hist[i] = 0;
      }
    }
    __syncthreads();
    ...
    if (tid == 0) {
      ...
      CUTE_UNROLL
      for (int r = 0; r < BM; ++r) {
        int len = refresh_locked_s[r] ? (refresh_end_s[r] - refresh_start_s[r]) : 0;
        total += len;
        refresh_prefix_s[r + 1] = total;
      }
    }
    __syncthreads();
    ...
    __syncthreads();
#else
#if HOPPER_RFB_HIST_MATRIX_ADD
    constexpr int REFRESH_THREADS_PER_ROW = 4;  // warp-aligned row groups; no shared atomic
#else
    constexpr int REFRESH_THREADS_PER_ROW = THREADS / BM;  // 384 / 64 = 6
#endif
    int row_local = tid / REFRESH_THREADS_PER_ROW;
    int sub = tid - row_local * REFRESH_THREADS_PER_ROW;
    if (row_local < BM) {
      int row = cta_m_base + row_local;
      if (sub == 0) {
        int locked = 0;
    ...
    if (row_local < BM && refresh_locked_s[row_local]) {
      int32_t* hrow = refresh_hist + row_local * REFRESH_NB_MAX;
      for (int b = sub; b < REFRESH_NB_MAX; b += REFRESH_THREADS_PER_ROW) {
        hrow[b] = 0;
      }
    }
    __syncthreads();
    ...
    if (row_local < BM && refresh_locked_s[row_local]) {
      int row = cta_m_base + row_local;
      int start = refresh_start_s[row_local];
      int scan_to = refresh_end_s[row_local];
      int32_t* hrow = refresh_hist + row_local * REFRESH_NB_MAX;
#if HOPPER_RFB_HIST_MATRIX_ADD
      int lane_id = tid & 31;
      int group_lane_base = lane_id & ~(REFRESH_THREADS_PER_ROW - 1);
      unsigned group_mask = ((1u << REFRESH_THREADS_PER_ROW) - 1u) << group_lane_base;
      for (int p = start + sub;; p += REFRESH_THREADS_PER_ROW) {
        int active = p < scan_to;
        unsigned active_mask = __ballot_sync(group_mask, active);
        if (active_mask == 0) break;
        if (active) {
          int b = 0;
#if HOPPER_SPARSE_REFRESH_FROM_BUF == 2
...
#else
      for (int p = start + sub; p < scan_to; p += REFRESH_THREADS_PER_ROW) {
        int b = 0;
#if HOPPER_SPARSE_REFRESH_FROM_BUF == 2
        b = (int)buf_bucket[(size_t)row * BUF + p];
#else
    ...
    if (row_local < BM && refresh_locked_s[row_local] && sub == 0) {
      int row = cta_m_base + row_local;
      int32_t* hrow = refresh_hist + row_local * REFRESH_NB_MAX;
      int gate = th[row];
#if !HOPPER_RFB_EARLY_UNLOCK
...
#else
  if (warpgroup == 0 && blockIdx.y > 0 && (blockIdx.y % THR_REFRESH_GROUP) == 0) {
...
void hopper_smalln_score_to_sparse_m64n8_full_kernel(
  ...
  // 8-row register queue state.  val/idx are spread across the 32 warp lanes
  // (lane L holds slot L of the row's queue); rq8_cnt_dist is a single
  // warp-distributed register where lane r holds row r's queue length.
  __half rq8_val0 = __float2half(0.0f), rq8_val1 = __float2half(0.0f);
  __half rq8_val2 = __float2half(0.0f), rq8_val3 = __float2half(0.0f);
  __half rq8_val4 = __float2half(0.0f), rq8_val5 = __float2half(0.0f);
  __half rq8_val6 = __float2half(0.0f), rq8_val7 = __float2half(0.0f);
  int rq8_idx0 = 0, rq8_idx1 = 0, rq8_idx2 = 0, rq8_idx3 = 0;
  int rq8_idx4 = 0, rq8_idx5 = 0, rq8_idx6 = 0, rq8_idx7 = 0;
  int rq8_cnt_dist = 0;
#define RQ8_FIND_NTH_BIT(MASK, TARGET) __fns((MASK), 0, (TARGET) + 1)
#define RQ8_FLUSH(QR, VAL, IDX)                                              \
  do {                                                                       \
    if ((QR) < logical_rows) {                                               \
      unsigned rq8_mask = __activemask();                                    \
      int rq8_cnt = __shfl_sync(rq8_mask, rq8_cnt_dist, (QR));               \
      int rq8_row = query_base + (QR);                                       \
      int rq8_base = 0;                                                      \
      if (lane == 0 && rq8_cnt > 0) rq8_base = atomicAdd(&qcount[rq8_row], rq8_cnt); \
      rq8_base = __shfl_sync(rq8_mask, rq8_base, 0);                         \
      int rq8_wc = rq8_cnt;                                                  \
      if (rq8_base + rq8_wc > BUF) rq8_wc = BUF - rq8_base;                  \
      if (lane < rq8_wc) {                                                   \
        int rq8_pos = rq8_base + lane;                                       \
        if (rq8_pos < BUF) {                                                 \
          buf_val[(size_t)rq8_row * BUF + rq8_pos] = (VAL);                  \
          buf_idx[(size_t)rq8_row * BUF + rq8_pos] = (IDX);                  \
        }                                                                    \
      }                                                                      \
      if (lane == (QR)) rq8_cnt_dist = 0;                                    \
    }                                                                        \
  } while (0)
#define RQ8_APPEND(QR, VAL, IDX, PEERS, PEER_N)                              \
  do {                                                                       \
    unsigned rq8_amask = __activemask();                                     \
    int rq8_cur = __shfl_sync(rq8_amask, rq8_cnt_dist, (QR));                \
    if (rq8_cur + (PEER_N) > 32) { RQ8_FLUSH((QR), (VAL), (IDX)); rq8_cur = 0; } \
    int rq8_rank = lane - rq8_cur;                                           \
    int rq8_do = (rq8_rank >= 0 && rq8_rank < (PEER_N));                     \
    int rq8_src = rq8_do ? RQ8_FIND_NTH_BIT((PEERS), rq8_rank) : 0;          \
    float rq8_svf = __shfl_sync(rq8_amask, vf, rq8_src);                     \
    int rq8_sidx = __shfl_sync(rq8_amask, global_base, rq8_src);             \
    if (rq8_do) { (VAL) = __float2half(rq8_svf); (IDX) = rq8_sidx; }         \
    if (lane == (QR)) rq8_cnt_dist = rq8_cur + (PEER_N);                     \
  } while (0)
#define RQ8_APPEND_MASKED(QR, VAL, IDX, MASK)                               \
  do {                                                                       \
    if ((QR) < logical_rows) {                                               \
      unsigned rq8_peers = pred_mask & (MASK);                               \
      int rq8_peer_n = __popc(rq8_peers);                                    \
      if (rq8_peer_n != 0) RQ8_APPEND((QR), (VAL), (IDX), rq8_peers, rq8_peer_n); \
    }                                                                        \
  } while (0)
#endif  // HOPPER_M8_REG_ROW_QUEUE
...
#endif
          if ((i & 1) == 0) {
            RQ8_APPEND_MASKED(0, rq8_val0, rq8_idx0, 0x11111111u);
            RQ8_APPEND_MASKED(2, rq8_val2, rq8_idx2, 0x22222222u);
            RQ8_APPEND_MASKED(4, rq8_val4, rq8_idx4, 0x44444444u);
            RQ8_APPEND_MASKED(6, rq8_val6, rq8_idx6, 0x88888888u);
          } else {
            RQ8_APPEND_MASKED(1, rq8_val1, rq8_idx1, 0x11111111u);
            RQ8_APPEND_MASKED(3, rq8_val3, rq8_idx3, 0x22222222u);
            RQ8_APPEND_MASKED(5, rq8_val5, rq8_idx5, 0x44444444u);
            RQ8_APPEND_MASKED(7, rq8_val7, rq8_idx7, 0x88888888u);
          }
        }
#else
...
#if HOPPER_M8_REG_ROW_QUEUE
  RQ8_FLUSH(0, rq8_val0, rq8_idx0);
  RQ8_FLUSH(1, rq8_val1, rq8_idx1);
  RQ8_FLUSH(2, rq8_val2, rq8_idx2);
  RQ8_FLUSH(3, rq8_val3, rq8_idx3);
  RQ8_FLUSH(4, rq8_val4, rq8_idx4);
  RQ8_FLUSH(5, rq8_val5, rq8_idx5);
  RQ8_FLUSH(6, rq8_val6, rq8_idx6);
  RQ8_FLUSH(7, rq8_val7, rq8_idx7);
#undef RQ8_APPEND_MASKED
#undef RQ8_APPEND
#undef RQ8_FLUSH
#undef RQ8_FIND_NTH_BIT
#endif  // HOPPER_M8_REG_ROW_QUEUE
}
  ...
  for (int c_rel = 0; c_rel < C_TILES_PER_CTA; ++c_rel) {
    ...
    if (warpgroup == owner_wg && base_tile < total_base_tiles) {
      ...
      for (int i = 0; i < cute::size(cute_tCrC); ++i) {
        ...
        int b = braw < 0 ? 0 : (braw > NB - 1 ? NB - 1 : braw);
        int pred = valid && (b <= th[row]);
#if HOPPER_GQA_WARP_COMPACT_WRITE
        unsigned active_mask = __activemask();
        unsigned pred_mask = __ballot_sync(active_mask, pred != 0);
        if (pred_mask != 0) {
#if !HOPPER_SPARSE_NO_HIST && HOPPER_SPARSE_REFRESH_FROM_BUF == 0
...
#define FT_GQA_M8_FULL_SPARSE_LAUNCH(KS)                                        \
  do {                                                                          \
    ...
    if (!g_gqa_m8_sparse_attr[KS]) {                                            \
      cudaFuncSetAttribute(hopper_gqa_smalln_score_to_sparse_m64n8_full_kernel<KS>, \
                           cudaFuncAttributeMaxDynamicSharedMemorySize, _smem); \
      g_gqa_m8_sparse_attr[KS] = true;                                          \
    }                                                                           \
    dim3 grid(base_tile_groups, hkv, 1);                                         \
    hopper_gqa_smalln_score_to_sparse_m64n8_full_kernel<KS><<<grid, dim3(THREADS, 1, 1), _smem, stream>>>( \
