...
inline FTLaunchParams ft_compute_params(int B, int N, int k, int sm_count) {
    ...
    // chunk_spread: num_seg contiguous 256-blocks spread evenly over the row.
    //   sample_stride = N/num_seg (>=256): 32 lanes consecutive => coalesced
    //   within each block; only block starts are spread => global sampling,
    //   representative threshold for any distribution (fixes prefix-only cliff).
    int S_target = k > 32768 ? k : 32768;
    if (S_target > N) S_target = N;
    int num_seg = (S_target + FT_BLOCK_THREADS - 1) / FT_BLOCK_THREADS;
    if (num_seg < 1) num_seg = 1;
    uint32_t seg_stride = (uint32_t)(N / num_seg);
    if (seg_stride < (uint32_t)FT_BLOCK_THREADS) seg_stride = (uint32_t)FT_BLOCK_THREADS;   // 前缀：段紧挨着排
