"""Compare the H100 LiteTopK MS MARCO operator with a dense reference.
...
See the parent H100 README for the CUDA include paths required by the JIT
build. This benchmark must be run on an H100-class GPU.
"""
...
from litetopk_ops import fused_ip_sparse_h100  # noqa: E402
...
def main():
    ...
    if args.refresh < 0:
        args.refresh = 8
    # Fix the benchmark query tile unless the caller explicitly supplies
    # another supported value. The H100 host wrapper uses BM=128.
    ...
    if baseline_kind == "engine_dense":
        ...
        _ops._ensure_loaded()
        def baseline():
            return torch.ops.litetopk_sm90.dense_scores(q, kv3, 0).topk(k, dim=-1).indices
    ...
    print(f"  LiteTopK SM90 fused        : {t_litetopk:.4f} ms   speedup {t_base / t_litetopk:.2f}x", flush=True)
