#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Per-(chunk, seq) config sweep for the DSA fused E2E path.

Sample size, bucket count (NB) and refresh cadence all trade off sample-stage
cost vs. main-scan cost vs. candidate density. This grid-searches them and, for
each (chunk, seq), reports the FASTEST end-to-end config whose recall clears a
...
Env:
  ...
  K        default 2048
  RECMIN   default 99.0   (min recall %% to accept a config)
  WARMUP   default 3      ITERS default 8   (timing loops during sweep)
...
def run(tag, chunk, sp, dn):
    ...
    # reference top-k (exact deep_gemm), row-batched
    ...
    def recall(idx):  # idx [Q,K] long; vectorized set-overlap
        p = torch.searchsorted(refs, idx).clamp(max=K - 1)
        return 100.0 * (torch.gather(refs, 1, p) == idx).sum().item() / (Q * K)

    # B: ideal KV-split dense + torch.topk (config independent)
    ...
    for sample, NB, refresh in itertools.product(SAMPLES, BUCKETS, REFRESHS):
        ...
        # Sample-stage top-k-min via our radix-select CUDA kernel (replaces the
        # slow torch.topk). col_full carries the corpus positions so out_idx is
        # already the seed index; cnt_full=head means "all sample cols valid".
...
if __name__ == "__main__":
    ...
    print("\n============== DSA E2E BEST-CONFIG SWEEP (H100, K=%d, recall>=%.1f%%) ==============" % (K, RECMIN))
