#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Standard 3-way benchmark for the DSA fused sparse top-k kernel (dsa_marsco).

Compares, on the same H100, at 100% recall vs the exact deep_gemm top-k:
  A  stock          : deep_gemm.fp8_mqa_logits (SM90, no KV-split) + torch.topk
...
sparse-selection win (no 268MB dense logits, no million-wide torch.topk).

Usage:
    LD_LIBRARY_PATH=/usr/local/cuda-12.9/compat:$LD_LIBRARY_PATH \
    TORCH_CUDA_ARCH_LIST=9.0a python3 bench_3way.py [256k 512k 768k 1m]
...
def build():
    ...
    sparse = load(name="github_dsa_sparse", sources=[SRC], extra_include_paths=INC,
                  extra_cuda_cflags=FLAGS, build_directory="/tmp/build_sparse",
                  extra_ldflags=["-lcuda"], verbose=False)
    dense = load(name="github_dsa_dense", sources=[SRC], extra_include_paths=INC,
                 extra_cuda_cflags=FLAGS + ["-DDENSE_WRITE"], build_directory="/tmp/build_dense",
                 extra_ldflags=["-lcuda"], verbose=False)
    return sparse, dense
...
if __name__ == "__main__":
    ...
    print("\n================ 3-WAY BENCHMARK (H100, K=2048, recall=100%) ================")
    ...
    print("\nBreakdown (ms):")
    print(f"{'seq':>5} {'B_score':>8} {'O_sample':>9} {'O_score':>8} {'O_select':>9}  (B_score=dense logits write; O_sample=sample/prep)")
    for tag, r in rows:
