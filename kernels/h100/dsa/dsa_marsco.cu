// Route-A stage 2 host wrapper: launch the DSA ReLU-MQA scoring kernel with a
// sample+write epilogue. Scoring math is reused from deep_gemm; only the final
// dense-store epilogue is replaced by compact candidate writes.
...
#endif
#ifndef DSA_BLOCK_KV
#define DSA_BLOCK_KV 128
#endif
#ifndef DSA_MATH_THREADS
#define DSA_MATH_THREADS 512
#endif
...
// The scan already converged a per-row gate bucket `th` (candidates with bucket
// <= th are ALL in the buffer, because the gate only tightens over time). So we
// avoid a full radix over every candidate:
...
__global__ void compact_topk_min_thr_marsco_kernel(
    ...
    // Membership in the radix "select set": boundary bucket only in the fast
    // path; all valid candidates when underfilled; bucket<=th otherwise.
    #define DSA_IN_SELECT_SET(b) \
        (s_use_boundary ? ((b) == th) : (s_select_all ? true : ((b) <= th)))

    // Pass 2..5: in-block 4-pass radix over the select set to find the k_eq-th
    // smallest encoded value (= pivot).
    uint32_t mask = 0u;
    #pragma unroll
    for (int pass = 0; pass < 4; ++pass) {
        int shift = 24 - pass * 8;
    ...
    // Count enc<pivot within the select set (these are the guaranteed winners
    // inside the boundary), then decide how many pivot-equal ties to admit.
    ...
    // Write pass: bucket<th copied straight through (fast path); the select set
    // fills the rest via pivot partition + tie truncation.
    for (int j = tid; j < n; j += BT) {
        float v = vrow[j];
        if (!isfinite(v)) continue;
        int braw = static_cast<int>((v - o) * inv);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> mqa_logits_dsa_marsco(
    ...
    (void)cand_cap64;
    // Match the previous hopper/group-indexed sparse path: use global M as the
    // compact buffer capacity. This removes cap truncation as a variable; only
    // cand_cnt[row] defines the valid prefix, so tail slots remain uninitialized.
    ...
    // Do not initialize tail slots. Selection is qcount-bounded, so only
    // cand_val/cand_idx[:, :cand_cnt[row]] are ever read.
    ...
    const int smem_ks = BLOCK_KV * esz_f32;
#if DSA_WARP_QUEUE
...
#endif
    const int smem = NUM_Q_STAGES * smem_q + NUM_KV_STAGES * smem_kv +
                     NUM_Q_STAGES * smem_w + NUM_KV_STAGES * smem_ks +
                     (NUM_Q_STAGES * 2 + NUM_KV_STAGES * 2 + (MATH_THREADS / 128) * 2) * 8 + 4 +
                     smem_warpq;

    auto kernel = &dsa_marsco::sm90_dsa_marsco<
    ...
    // KV-split grid: x = q-blocks, y = KV splits, chosen so x*y ~= NUM_SMS to use
    // all SMs. Persistent-CTA Q-only scheduling left <=16 SMs busy at Q=64.
    ...
    if (num_kv_splits_override > 0) {
        ...
        // Load-balanced auto = OVER-SUBSCRIPTION. Empirically (256k-1M sweep),
        // targeting ~4 CTA-waves per SM beats "exactly fill the SMs": more, smaller
        // CTAs give better tail balancing when NUM_SMS is not a multiple of the CTA
        // count, and more concurrent waves hide TMA/epilogue latency. Aim for
        // num_q_blocks * splits ~= kWaves * NUM_SMS, capped by total_kv_blocks and
        // a minimum KV per CTA so tiny sequences do not over-split.
        constexpr int kWaves = 4;
        const int qb = num_q_blocks > 0 ? num_q_blocks : 1;
        num_kv_splits = (kWaves * NUM_SMS + qb - 1) / qb;   // ceil(kWaves*NUM_SMS / num_q_blocks)
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> mqa_logits_dsa_marsco_out(
    ...
    // External reusable buffers. Tail slots are intentionally left uninitialized;
    // selection is cand_cnt-bounded. Only counters/histogram and the seed prefix
    // are reset each call.
...
// Threshold-aware select: reuses the scan's converged gate bucket (th_bucket)
// plus the bucket coordinates (origin, inv_delta) to do bucket<th direct-write +
// bucket==th boundary radix, mirroring tidal/marsco's flashtopk_select_thr path.
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("mqa_logits_dsa_marsco", &mqa_logits_dsa_marsco,
          "DSA ReLU-MQA scoring with bucket-gated direct atomic epilogue",
    ...
    m.def("mqa_logits_dsa_marsco_out", &mqa_logits_dsa_marsco_out,
          "DSA ReLU-MQA scoring into externally reusable candidate buffers",
          pybind11::arg("q"), pybind11::arg("kv"), pybind11::arg("kv_scales"),
          pybind11::arg("weights"), pybind11::arg("cu_start"), pybind11::arg("cu_end"),
          pybind11::arg("origin"), pybind11::arg("inv_delta"), pybind11::arg("th_bucket"),
          pybind11::arg("seed_val"), pybind11::arg("seed_idx"),
