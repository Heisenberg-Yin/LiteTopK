...
#define DSA_TMEM_BUFS 2
    ...
    const auto load_schedule = [&]() -> cute::tuple<uint32_t, uint32_t, uint32_t, uint32_t> {
        ...
        for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
            ...
            if (block_q_idx * BLOCK_Q + i >= seq_len) {
                // Padded row of a ragged final q-block (seq_len % BLOCK_Q != 0):
                // force an empty KV range so its (zero-filled TMA q) scores can
                // never pass the gate -> no candidate/bcount writes at
                // row_q >= seq_len (OOB). The (S, 0) pair is neutral for the
                // start/end aggregation below.
    ...
    if (is_tma_load_warp) {
        ...
        if (block_q_idx < num_q_blocks) {
            ...
            #pragma unroll
            for (uint32_t i = 0; i < BLOCK_Q; ++ i) {
                const uint32_t rq = min(block_q_idx * BLOCK_Q + i, seq_len - 1);  // clamp: ragged final block
                o_reg[i]   = origin[rq];
