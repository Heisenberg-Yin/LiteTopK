...
They require CUDA and CUTLASS/CuTe headers in addition to the C++ standard
library.
...
Local derivative of upstream's `sm100/prefill/sparse/fwd/head128`:
...
- `sm100/prefill/sparse/fwd/head128_fp8/` — an fp8 (e4m3) variant of the
  ...
  cores, and a query-major membership bitmask restores each query's
  attend-set inside the kernel. Includes `mma_fp8_noelect.h`, a 2-SM fp8
  MMA atom written for this variant.

`head128_fp8/` is maintained locally and is not automatically synchronized
with upstream FlashMLA.
