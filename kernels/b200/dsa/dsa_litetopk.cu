    ...
    if (s_have_boundary_meta &&
        ...
        for (int tile = 0; tile < n; tile += BT) {
            ...
            const uint32_t th_code =
                __float_as_uint(static_cast<float>(th)) >> 8;
...
void launch_seed_prep(
    ...
    seed_prep_kernel<<<Q, kSeedThreads, seed_smem, stream>>>(
        ...
        origin, inv_delta, th_bucket, cand_cnt);
}
...
void seed_prep_litetopk_(torch::Tensor slog, int64_t num_buckets64, int64_t topk64,
    ...
    launch_seed_prep(
        ...
        th_bucket.data_ptr<int32_t>(), cand_cnt.data_ptr<int32_t>(), stream);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void carry_votes_topk_reset_litetopk_(
    ...
    TORCH_CHECK(
        ...
        "k must be in [1,8192]");
    TORCH_CHECK(
        max_vote64 >= 1 && max_vote64 <= kCarryMaxK,
    ...
    const int bins = max_vote + 1;
    const int blocks =
        (count + kCarryTileItems - 1) / kCarryTileItems;
    TORCH_CHECK(
        out_idx.numel() == out_k,
    ...
    carry_votes_plan_litetopk_kernel<<<
    ...
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    carry_votes_emit_reset_litetopk_kernel<<<
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def(
        "dense_hist_meta_litetopk_",
        &litetopk_dense::dense_hist_meta_litetopk_,
        "Exact FP16-coarse histogram metadata for dense prefill top-k",
        pybind11::arg("logits"),
        pybind11::arg("row_starts"),
        pybind11::arg("row_ends"),
        pybind11::arg("threshold"),
        pybind11::arg("count_lt"),
        pybind11::arg("count_eq"),
        pybind11::arg("rows"),
        pybind11::arg("stride0"),
        pybind11::arg("stride1"),
        pybind11::arg("topk"),
    ...
    m.def(
        "dense_prehist_select_litetopk_",
