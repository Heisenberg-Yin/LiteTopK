...
static PagedCandidateShape check_paged_candidate_layout(
  ...
  TORCH_CHECK(
      logical_cap64 >= 1 && logical_cap64 < std::numeric_limits<int32_t>::max(),
...
static SegmentCandidateShape check_segment_candidate_layout(
    const torch::Tensor& seed_val, const torch::Tensor& seed_idx,
    const torch::Tensor& segment_val, const torch::Tensor& segment_idx,
    const torch::Tensor& segment_desc) {
  TORCH_CHECK(seed_val.is_cuda() && seed_idx.is_cuda() &&
                  segment_val.is_cuda() && segment_idx.is_cuda() &&
                  segment_desc.is_cuda(),
              "segmented candidate tensors must be CUDA");
  TORCH_CHECK(seed_val.device() == seed_idx.device() &&
  ...
  TORCH_CHECK(seed_val.is_contiguous() && seed_idx.is_contiguous() &&
  ...
  TORCH_CHECK(seed_idx.scalar_type() == torch::kInt &&
  ...
  TORCH_CHECK(seed_val.dim() == 2 && seed_val.size(0) > 0 &&
  ...
  TORCH_CHECK(segment_desc.dim() == 3 &&
  ...
  const int64_t q_blocks =
      (rows + kSegmentCandidateBlockQ - 1) / kSegmentCandidateBlockQ;
  const int64_t micro_records =
      q_blocks * kSegmentCandidateDescriptorWarps * kSegmentCandidateMicro;
  TORCH_CHECK(
      segment_val.numel() > micro_records &&
          (segment_val.numel() - micro_records) % kSegmentCandidatePage == 0,
      "segment arena must be micro_records + P*4096 with P>0");
  const int64_t pool_pages =
      (segment_val.numel() - micro_records) / kSegmentCandidatePage;
  TORCH_CHECK(
      pool_pages > 0 && segment_val.numel() < kSegmentCandidateInvalidStart,
      "segmented physical arena exceeds the uint32 descriptor ABI");
  TORCH_CHECK(!tensor_storage_ranges_overlap(seed_val, seed_idx) &&
...
static void check_segment_spill_layout(const torch::Tensor& spill_desc,
                                       const torch::Tensor& row_spill,
                                       const torch::Tensor& segment_desc,
                                       const SegmentCandidateShape& shape) {
  TORCH_CHECK(spill_desc.is_cuda() && row_spill.is_cuda() &&
  ...
  TORCH_CHECK(spill_desc.is_contiguous() && row_spill.is_contiguous(),
              "spill descriptors/row flags must be contiguous");
  TORCH_CHECK(spill_desc.scalar_type() == torch::kLong &&
                  row_spill.scalar_type() == torch::kInt,
              "spill_desc must be int64 and row_spill must be int32");
  TORCH_CHECK(spill_desc.dim() == 4 && spill_desc.size(0) == shape.rows &&
  ...
  TORCH_CHECK(row_spill.dim() == 1 && row_spill.numel() == shape.rows,
              "row_spill must have Q int32 elements");
  TORCH_CHECK(!tensor_storage_ranges_overlap(spill_desc, segment_desc) &&
...
__global__ void online_fixed_init_litetopk_kernel(
    ...
    int32_t* __restrict__ boundary_meta, int rows) {
...
__device__ __forceinline__ void finalize_static_hot_mapped_paged_record(
    const CandidateValue value, int32_t* __restrict__ packed_idx_ptr,
    const int32_t* __restrict__ index_map, const int index_map_size,
    const int num_buckets, uint32_t* __restrict__ hist,
  ...
  if (original_idx < 0 || original_idx >= index_map_size ||
      static_cast<uint32_t>(original_idx) > dsa_litetopk::kCandidateIndexMask) {
    atomicOr(scan_status, 16);
    return;
  }
  packed_idx = static_cast<int32_t>(
      (static_cast<uint32_t>(packed_idx) & ~dsa_litetopk::kCandidateIndexMask) |
      static_cast<uint32_t>(original_idx));
  *packed_idx_ptr = packed_idx;

  const float decoded = dsa_litetopk::candidate_decode_score(value, packed_idx);
  if (!isfinite(decoded)) {
    atomicOr(scan_status, 4);
    return;
  }
...
__global__ void finalize_static_hot_meta_mapped_paged_litetopk_kernel(
    ...
    int32_t* __restrict__ overflow_idx, const int32_t* __restrict__ page_table,
    const int32_t* __restrict__ cand_cnt, int32_t* __restrict__ th_bucket,
    int32_t* __restrict__ boundary_meta, int32_t* __restrict__ status,
    const int32_t* __restrict__ index_map, int index_map_size, int rows,
    int overflow_pages, int table_pages_per_row, int logical_cap,
    int num_buckets, int topk) {
  constexpr int kThreads = 256;
  constexpr int kBins = 256;
  const int row = static_cast<int>(blockIdx.x);
  ...
  for (int j = tid; j < inline_n; j += kThreads) {
    ...
    finalize_static_hot_mapped_paged_record(
        inline_val[offset], inline_idx + offset, index_map, index_map_size,
        num_buckets, hist, &scan_status);
  }
  __syncthreads();

  const int overflow_n = max(n - kPagedCandidateInline, 0);
  const int num_pages =
      (overflow_n + kPagedCandidatePage - 1) >> kPagedCandidatePageShift;
  for (int logical_page = 0; logical_page < num_pages && scan_status == 0;
       ++logical_page) {
    if (tid == 0) {
      const int id =
          logical_page < table_pages_per_row
              ? page_table[static_cast<uint64_t>(row) * table_pages_per_row +
                           logical_page]
              : -1;
      page_id = id;
    ...
    const int page_n = min(overflow_n - logical_page * kPagedCandidatePage,
                           kPagedCandidatePage);
    ...
    for (int j = tid; j < page_n; j += kThreads) {
      ...
      finalize_static_hot_mapped_paged_record(
          overflow_val[offset], overflow_idx + offset, index_map,
          index_map_size, num_buckets, hist, &scan_status);
    }
    __syncthreads();
  }
...
__device__ __forceinline__ bool segment_candidate_descriptor_valid(
    const uint32_t physical_start, const uint32_t count, const int row,
    const int producer_warp, const int micro_records, const int pool_pages,
    const int physical_records, const uint32_t allocated_pages) {
  if (count == 0u) {
  ...
  if (physical_start == kSegmentCandidateInvalidStart ||
      count > static_cast<uint32_t>(kSegmentCandidateDescriptorMaxCount) ||
  ...
  if (physical_start < static_cast<uint32_t>(micro_records)) {
    return physical_start >= micro_base &&
           physical_start + count <= micro_base + kSegmentCandidateMicro;
  }
  const uint32_t pooled = physical_start - static_cast<uint32_t>(micro_records);
  const uint32_t page = pooled / kSegmentCandidatePage;
  const uint32_t page_offset = pooled % kSegmentCandidatePage;
  return page < static_cast<uint32_t>(pool_pages) && page < allocated_pages &&
         page_offset + count <= kSegmentCandidatePage;
}

__device__ __forceinline__ bool segment_candidate_spill_descriptor_valid(
    const uint32_t physical_start, const uint32_t count,
    const int micro_records, const int pool_pages, const int physical_records,
  ...
  return pooled % kSegmentCandidatePage == 0u &&
         page < static_cast<uint32_t>(pool_pages) && page < allocated_pages;
}
...
__global__ void finalize_static_hot_meta_mapped_segmented_litetopk_kernel(
    const CandidateValue* __restrict__ seed_val, int32_t* __restrict__ seed_idx,
    ...
    const int32_t* __restrict__ writer_status, int32_t* __restrict__ total_cnt,
    int32_t* __restrict__ th_bucket, int32_t* __restrict__ boundary_meta,
    int32_t* __restrict__ status, const int32_t* __restrict__ index_map,
    int index_map_size, int rows, int desc_windows, int micro_records,
    int pool_pages, int physical_records, int logical_cap, int num_buckets,
    int topk) {
  constexpr int kThreads = 256;
  const int row = static_cast<int>(blockIdx.x);
  const int tid = static_cast<int>(threadIdx.x);
  ...
  const int seed_n =
      raw_seed_n < 0 ? 0 : min(raw_seed_n, kPagedCandidateInline);
  if (tid == 0) {
    int st = 0;
    ...
    allocated_pages =
        raw_pool_head < 0 ? 0u : static_cast<uint32_t>(raw_pool_head);
  ...
  const uint64_t seed_base = static_cast<uint64_t>(row) * kPagedCandidateInline;
  if (scan_status == 0) {
    for (int j = tid; j < seed_n; j += kThreads) {
      const uint64_t offset = seed_base + j;
      finalize_static_hot_mapped_paged_record(
          seed_val[offset], seed_idx + offset, index_map, index_map_size,
          num_buckets, hist, &scan_status);
    }
  }
  ...
  for (int window = 0; window < desc_windows; ++window) {
    ...
    if (lane == 0) {
      const uint64_t descriptor =
          segment_desc[(static_cast<uint64_t>(row) * desc_windows + window) *
      ...
      valid = segment_candidate_descriptor_valid(
          physical_start, count, row, producer_warp, micro_records, pool_pages,
          physical_records, allocated_pages);
      if (!valid) {
        atomicOr(&scan_status, kSegmentCandidateInvalidDescriptorStatus);
        count = 0u;
      }
    ...
    for (uint32_t j = static_cast<uint32_t>(lane); j < count; j += 32u) {
      const uint64_t offset = static_cast<uint64_t>(physical_start) + j;
      finalize_static_hot_mapped_paged_record(
          segment_val[offset], segment_idx + offset, index_map, index_map_size,
          num_buckets, hist, &scan_status);
    }
  }
  if (has_spill) {
    for (int window = 0; window < desc_windows; ++window) {
#pragma unroll
      for (int spill_slot = 0; spill_slot < kSegmentCandidateSpillSlots;
           ++spill_slot) {
        uint32_t physical_start = 0u;
        uint32_t count = 0u;
        if (lane == 0) {
          const uint64_t descriptor =
              spill_desc[((static_cast<uint64_t>(row) * desc_windows + window) *
                              kSegmentCandidateDescriptorWarps +
                          producer_warp) *
          ...
          physical_start = static_cast<uint32_t>(descriptor);
          count = static_cast<uint32_t>(descriptor >> 32);
          const bool valid = segment_candidate_spill_descriptor_valid(
              physical_start, count, micro_records, pool_pages,
              physical_records, allocated_pages);
          if (!valid) {
            atomicOr(&scan_status, kSegmentCandidateInvalidDescriptorStatus);
            count = 0u;
        ...
        for (uint32_t j = static_cast<uint32_t>(lane); j < count; j += 32u) {
          const uint64_t offset = static_cast<uint64_t>(physical_start) + j;
          finalize_static_hot_mapped_paged_record(
              segment_val[offset], segment_idx + offset, index_map,
              index_map_size, num_buckets, hist, &scan_status);
        }
      }
    }
...
__global__ void compact_static_hot_candidates_litetopk_kernel(
    CandidateValue* __restrict__ cand_val, int32_t* __restrict__ cand_idx,
    int32_t* __restrict__ cand_cnt, int32_t* __restrict__ th_bucket,
    int32_t* __restrict__ boundary_meta, int32_t* __restrict__ status, int rows,
    int cand_cap, int num_buckets, int topk) {
  constexpr int kThreads = 256;
...
#pragma unroll
  for (int pass = 0; pass < 3; ++pass) {
    ...
    for (int j = tid; j < n; j += kThreads) {
      ...
      const uint32_t code = dsa_litetopk::candidate_load_score_code(
    ...
    compact_find_radix_digit_warp0(exact_hist, &exact_desired, &exact_kfind,
                                   desired, shift, tid);
    __syncthreads();
  ...
  if (equal_take <= 0 || equal_take > topk || pivot >= 0x00ffffffu) {
    if (tid == 0) {
      status[row] = 8;
      cand_cnt[row] = cand_cap + 1;
      meta[0] = 0;
  ...
  for (int tile = 0; tile < n; tile += kThreads) {
    ...
    if (j < n) {
      ...
      code = dsa_litetopk::candidate_load_score_code(value, packed_idx);
      valid = true;
    }
    ...
    const int equal_rank =
        equal_base + __popc(equal_mask & ((1u << lane) - 1u));

    const bool keep =
        valid && (code < pivot || (is_equal && equal_rank < equal_take));
    ...
    const int output = warp_base + __popc(keep_mask & ((1u << lane) - 1u));
  ...
  if (tid == 0) {
    if (compact_count != topk || equal_seen < equal_take) {
...
#pragma unroll
    ...
    __shared__ int stat_smax[32], stat_exceed[32];
    const int wid = threadIdx.x >> 5;
    if ((threadIdx.x & 31) == 0) {
      stat_smax[wid] = stat_local_max;
      stat_exceed[wid] = stat_local_over;
    }
    __syncthreads();
    if (threadIdx.x == 0) {
      int m = 0, ov = 0;
...
__device__ __forceinline__ void compact_topk_paged_fast_scan_span(
    const CandidateValue* __restrict__ val, const int32_t* __restrict__ idx,
    const int count, const int th, const uint32_t th_code,
    const uint32_t next_th_code, int32_t* __restrict__ out_idx,
    uint32_t* __restrict__ boundary_val, int32_t* __restrict__ boundary_idx,
    int* __restrict__ lt_cursor, int* __restrict__ eq_cursor, const int topk) {
  constexpr int kThreads = 256;
  ...
  for (int tile = 0; tile < count; tile += kThreads) {
    ...
    if (valid) {
      packed_idx = idx[j];
      score_code = dsa_litetopk::candidate_load_score_code(val[j], packed_idx);
    }
    const bool is_lt = valid && th > 0 && score_code < th_code;
    const bool is_eq = valid && score_code < next_th_code &&
                       (th == 0 || score_code >= th_code);
    const unsigned lt_mask = __ballot_sync(kFullMask, is_lt);
    const unsigned eq_mask = __ballot_sync(kFullMask, is_eq);
    int warp_lt_base = 0;
    int warp_eq_base = 0;
    if (lane == 0) {
      const int lt_count = __popc(lt_mask);
    ...
    warp_lt_base = __shfl_sync(kFullMask, warp_lt_base, 0);
    warp_eq_base = __shfl_sync(kFullMask, warp_eq_base, 0);

    if (is_lt) {
      const int pos = warp_lt_base + __popc(lt_mask & lane_mask);
      if (pos < topk) {
        out_idx[pos] = dsa_litetopk::candidate_decode_index(packed_idx);
      }
    }
    if (is_eq) {
      const int pos = warp_eq_base + __popc(eq_mask & lane_mask);
      if (pos < kBoundaryCapacity) {
        ...
        boundary_idx[pos] = dsa_litetopk::candidate_decode_index(packed_idx);
      }
    }
  }
}

__device__ __forceinline__ void compact_topk_segment_fast_scan_warp(
    const CandidateValue* __restrict__ val, const int32_t* __restrict__ idx,
    const uint32_t count, const int th, const uint32_t th_code,
    const uint32_t next_th_code, int32_t* __restrict__ out_idx,
    uint32_t* __restrict__ boundary_val, int32_t* __restrict__ boundary_idx,
    int* __restrict__ lt_cursor, int* __restrict__ eq_cursor, const int topk) {
  constexpr int kBoundaryCapacity = 256;
...
__device__ __forceinline__ bool compact_topk_paged_is_boundary(
    const uint32_t score_code, const int th, const uint32_t th_code,
    const uint32_t next_th_code) {
  return score_code < next_th_code && (th == 0 || score_code >= th_code);
}

__device__ __forceinline__ void compact_topk_paged_radix_hist_span(
    const CandidateValue* __restrict__ val, const int32_t* __restrict__ idx,
    const int count, const int th, const uint32_t th_code,
    const uint32_t next_th_code, const uint32_t prefix_mask,
    const uint32_t desired_prefix, const int shift,
  ...
  for (int j = tid; j < count; j += kThreads) {
    ...
    const uint32_t score_code =
        dsa_litetopk::candidate_load_score_code(val[j], packed_idx);
    if (compact_topk_paged_is_boundary(score_code, th, th_code, next_th_code) &&
        (score_code & prefix_mask) == (desired_prefix & prefix_mask)) {
      atomicAdd(&hist[(score_code >> shift) & 0xffu], 1u);
    }
  }
}

__device__ __forceinline__ void compact_topk_paged_count_pivot_lt_span(
    const CandidateValue* __restrict__ val, const int32_t* __restrict__ idx,
    const int count, const int th, const uint32_t th_code,
    const uint32_t next_th_code, const uint32_t pivot,
  ...
  for (int j = tid; j < count; j += kThreads) {
    ...
    const uint32_t score_code =
        dsa_litetopk::candidate_load_score_code(val[j], packed_idx);
    local_lt +=
        compact_topk_paged_is_boundary(score_code, th, th_code, next_th_code) &&
        score_code < pivot;
  }
  if (local_lt != 0) atomicAdd(pivot_lt, local_lt);
}

__device__ __forceinline__ void compact_topk_paged_write_pivot_span(
    const CandidateValue* __restrict__ val, const int32_t* __restrict__ idx,
    const int count, const int th, const uint32_t th_code,
    const uint32_t next_th_code, const uint32_t pivot, const int output_base,
    const int pivot_lt, const int eq_take, const int topk,
    int32_t* __restrict__ out_idx, int* __restrict__ write_lt,
  ...
  for (int tile = 0; tile < count; tile += kThreads) {
    ...
    if (is_lt) {
      const int rank = warp_lt_base + __popc(lt_mask & lane_mask);
    ...
    if (is_eq) {
      const int rank = warp_eq_base + __popc(eq_mask & lane_mask);
      if (rank < eq_take) {
...
static __device__ __noinline__ void compact_topk_paged_large_boundary_select(
    const CandidateValue* __restrict__ inline_val,
    const int32_t* __restrict__ inline_idx,
    const CandidateValue* __restrict__ overflow_val,
    const int32_t* __restrict__ overflow_idx,
    const int32_t* __restrict__ page_table, const int row, const int raw_n,
    const int overflow_pages, const int table_pages_per_row, const int th,
    const uint32_t th_code, const uint32_t next_th_code, const int output_base,
    const int k_target, const int topk, int32_t* __restrict__ row_out,
    int* __restrict__ page_id, uint32_t* __restrict__ hist,
    uint32_t* __restrict__ desired, uint32_t* __restrict__ kfind,
    int* __restrict__ pivot_lt, int* __restrict__ write_lt,
    int* __restrict__ write_eq) {
  constexpr int kThreads = 256;
  const int tid = static_cast<int>(threadIdx.x);
  ...
  const uint64_t inline_base =
      static_cast<uint64_t>(row) * kPagedCandidateInline;
  const int overflow_n = max(raw_n - kPagedCandidateInline, 0);
  const int num_pages =
      (overflow_n + kPagedCandidatePage - 1) >> kPagedCandidatePageShift;
...
#pragma unroll
  for (int pass = 0; pass < 3; ++pass) {
    ...
    compact_topk_paged_radix_hist_span(
        inline_val + inline_base, inline_idx + inline_base, inline_n, th,
        th_code, next_th_code, prefix_mask, desired_base, shift, hist);
    __syncthreads();
    ...
    for (int logical_page = 0; logical_page < num_pages; ++logical_page) {
      ...
      const uint64_t page_base =
          static_cast<uint64_t>(*page_id) * kPagedCandidatePage;
      compact_topk_paged_radix_hist_span(
          overflow_val + page_base, overflow_idx + page_base, page_n, th,
          th_code, next_th_code, prefix_mask, desired_base, shift, hist);
      __syncthreads();
    }

    compact_find_radix_digit_warp0(hist, desired, kfind, desired_base, shift,
                                   tid);
    __syncthreads();
  ...
  compact_topk_paged_count_pivot_lt_span(
      inline_val + inline_base, inline_idx + inline_base, inline_n, th, th_code,
      next_th_code, pivot, pivot_lt);
  __syncthreads();
  for (int logical_page = 0; logical_page < num_pages; ++logical_page) {
    if (tid == 0) {
      *page_id =
    ...
    const uint64_t page_base =
        static_cast<uint64_t>(*page_id) * kPagedCandidatePage;
    compact_topk_paged_count_pivot_lt_span(
        overflow_val + page_base, overflow_idx + page_base, page_n, th, th_code,
        next_th_code, pivot, pivot_lt);
    __syncthreads();
  }
  ...
  compact_topk_paged_write_pivot_span(
      inline_val + inline_base, inline_idx + inline_base, inline_n, th, th_code,
      next_th_code, pivot, output_base, exact_pivot_lt, eq_take, topk, row_out,
      write_lt, write_eq);
  __syncthreads();
  for (int logical_page = 0; logical_page < num_pages; ++logical_page) {
    if (tid == 0) {
    ...
    compact_topk_paged_write_pivot_span(
        overflow_val + page_base, overflow_idx + page_base, page_n, th, th_code,
        next_th_code, pivot, output_base, exact_pivot_lt, eq_take, topk,
        row_out, write_lt, write_eq);
    __syncthreads();
  }
...
__global__ void compact_topk_min_thr_paged_idx_out_litetopk_kernel(
    ...
    const int32_t* __restrict__ cand_cnt, const int32_t* __restrict__ th_in,
    const int32_t* __restrict__ boundary_meta, int rows, int overflow_pages,
    int table_pages_per_row, int logical_cap, int num_buckets, int topk,
    int32_t* __restrict__ out_idx, int32_t* __restrict__ votes, int votes_len,
    int vote_row_stride) {
  constexpr int kThreads = 256;
  ...
  if (tid == 0) {
    ...
    certificate_valid = tag < 0 && meta_th == th && th >= 0 &&
                        th < num_buckets && raw_n >= topk &&
                        raw_n <= logical_cap && meta_lt >= 0 &&
                        meta_lt < topk && meta_eq > 0 && need > 0 &&
                        need <= meta_eq && meta_lt + meta_eq <= raw_n;
    count_lt = meta_lt;
    count_eq = meta_eq;
    lt_cursor = 0;
    eq_cursor = 0;
  ...
  int32_t* const row_out = out_idx + static_cast<uint64_t>(row) * topk;
  ...
  compact_topk_paged_fast_scan_span(
      inline_val + inline_base, inline_idx + inline_base, inline_n, th, th_code,
      next_th_code, row_out, boundary_val, boundary_idx, &lt_cursor, &eq_cursor,
      topk);
  __syncthreads();
  ...
  for (int logical_page = 0; logical_page < num_pages; ++logical_page) {
    ...
    compact_topk_paged_fast_scan_span(
        overflow_val + page_base, overflow_idx + page_base, page_n, th, th_code,
        next_th_code, row_out, boundary_val, boundary_idx, &lt_cursor,
        &eq_cursor, topk);
  ...
  if (tid == 0) {
    certificate_valid = lt_cursor == count_lt && eq_cursor == count_eq;
  }
  __syncthreads();
  if (!certificate_valid) {
    asm volatile("trap;");
  ...
  if (boundary_n <= kBoundaryCapacity) {
    if (boundary_n == k_target) {
      ...
      dsa_litetopk_accumulate_inplace_votes(row_out, topk, tid, kThreads, votes,
                                            votes_len, row, vote_row_stride);
      return;
    }
    ...
    if (tid == 0) {
      ...
      desired = th == 0 ? 0u : (boundary_val[0] & 0xff0000u);
      kfind = static_cast<uint32_t>(k_target);
    }
    __syncthreads();
    ...
    for (int pass = 0; pass < 3; ++pass) {
      ...
      if (pass < num_passes) {
        ...
        if (tid < boundary_n) {
          ...
          if ((encoded & mask) == (desired_base & mask)) {
            atomicAdd(&hist[(encoded >> shift) & 0xffu], 1u);
          }
        }
        __syncthreads();
        compact_find_radix_digit_warp0(hist, &desired, &kfind, desired_base,
                                       shift, tid);
        __syncthreads();
        mask |= 0xffu << shift;
      }
...
__global__ void compact_topk_min_thr_paged_large_boundary_litetopk_kernel(
    const CandidateValue* __restrict__ inline_val,
    const int32_t* __restrict__ inline_idx,
    const CandidateValue* __restrict__ overflow_val,
    const int32_t* __restrict__ overflow_idx,
  ...
  const int tile_stride = static_cast<int>(gridDim.x) * kThreads;
  for (int tile_base = static_cast<int>(blockIdx.x) * kThreads;
       tile_base < rows; tile_base += tile_stride) {
    ...
    if (candidate_row < rows) {
      ...
      const int32_t* meta =
          boundary_meta + static_cast<uint64_t>(candidate_row) * num_buckets;
      const int tag = meta[0];
      const int meta_th = ~tag;
      const int meta_lt = meta[1];
      const int meta_eq = meta[2];
      const int need = topk - meta_lt;
      is_large = tag < 0 && meta_th == th && th >= 0 && th < num_buckets &&
                 raw_n >= topk && raw_n <= logical_cap && meta_lt >= 0 &&
                 meta_lt < topk && meta_eq > kBoundaryCapacity && need > 0 &&
                 need <= meta_eq && meta_lt + meta_eq <= raw_n;
    }
    if (is_large) {
    ...
    for (int task = 0; task < num_tasks; ++task) {
      ...
      compact_topk_paged_large_boundary_select(
          inline_val, inline_idx, overflow_val, overflow_idx, page_table, row,
          selected_raw_n, overflow_pages, table_pages_per_row, th, th_code,
          next_th_code, selected_output_base, selected_k_target, topk, row_out,
          &page_id, hist, &desired, &kfind, &pivot_lt, &write_lt, &write_eq);
      dsa_litetopk_accumulate_inplace_votes(row_out, topk, tid, kThreads, votes,
                                            votes_len, row, vote_row_stride);
      __syncthreads();
    }
...
__global__ void compact_topk_min_thr_segmented_idx_out_litetopk_kernel(
    const CandidateValue* __restrict__ seed_val,
    const int32_t* __restrict__ seed_idx, const int32_t* __restrict__ seed_cnt,
    const CandidateValue* __restrict__ segment_val,
    ...
    const int32_t* __restrict__ row_spill,
    const int32_t* __restrict__ segment_pool_head,
    const int32_t* __restrict__ total_cnt, const int32_t* __restrict__ th_in,
    const int32_t* __restrict__ boundary_meta,
    const int32_t* __restrict__ finalizer_status, int rows, int desc_windows,
    int micro_records, int pool_pages, int physical_records, int logical_cap,
    int num_buckets, int topk, int32_t* __restrict__ out_idx,
    int32_t* __restrict__ votes, int votes_len, int vote_row_stride) {
  constexpr int kThreads = 256;
  constexpr int kBoundaryCapacity = 256;
  const int row = static_cast<int>(blockIdx.x);
  const int tid = static_cast<int>(threadIdx.x);
  ...
  if (tid == 0) {
    ...
    const int need = topk - meta_lt;
    certificate_valid =
        finalizer_status[row] == 0 && tag < 0 && meta_th == th && th >= 0 &&
        th < num_buckets && raw_seed_n >= 0 &&
        raw_seed_n <= kPagedCandidateInline && spill_flag >= 0 &&
        raw_pool_head >= 0 && raw_pool_head <= pool_pages && raw_n >= topk &&
        raw_n <= logical_cap && meta_lt >= 0 && meta_lt < topk && meta_eq > 0 &&
        need > 0 && need <= meta_eq && meta_lt + meta_eq <= raw_n;
    count_lt = meta_lt;
    count_eq = meta_eq;
    lt_cursor = 0;
    eq_cursor = 0;
  ...
  const uint32_t next_th_code =
      dsa_litetopk::candidate_fp24_code(static_cast<float>(th + 1));
  const uint64_t seed_base = static_cast<uint64_t>(row) * kPagedCandidateInline;
  compact_topk_paged_fast_scan_span(seed_val + seed_base, seed_idx + seed_base,
                                    raw_seed_n, th, th_code, next_th_code,
                                    row_out, boundary_val, boundary_idx,
                                    &lt_cursor, &eq_cursor, topk);
  __syncthreads();
  ...
  for (int window = 0; window < desc_windows; ++window) {
    ...
    compact_topk_segment_fast_scan_warp(
        segment_val + physical_start, segment_idx + physical_start, count, th,
        th_code, next_th_code, row_out, boundary_val, boundary_idx, &lt_cursor,
        &eq_cursor, topk);
  }
  if (has_spill) {
    for (int window = 0; window < desc_windows; ++window) {
#pragma unroll
      for (int spill_slot = 0; spill_slot < kSegmentCandidateSpillSlots;
        ...
        count = __shfl_sync(0xffffffffu, count, 0);
        compact_topk_segment_fast_scan_warp(
            segment_val + physical_start, segment_idx + physical_start, count,
            th, th_code, next_th_code, row_out, boundary_val, boundary_idx,
            &lt_cursor, &eq_cursor, topk);
      }
    }
  }
...
__device__ __forceinline__ void compact_topk_segmented_descriptor_span(
    const uint64_t* __restrict__ segment_desc,
    const uint64_t* __restrict__ spill_desc, const int row,
    const int desc_windows, const int descriptor_id, const bool has_spill,
    const int micro_records, const int pool_pages, const int physical_records,
    const uint32_t allocated_pages, uint32_t* __restrict__ span_start,
    uint32_t* __restrict__ span_count, int* __restrict__ descriptors_valid) {
  const int base_descriptors = desc_windows * kSegmentCandidateDescriptorWarps;
  uint64_t descriptor = 0u;
  bool valid = false;
  if (descriptor_id < base_descriptors) {
    descriptor = segment_desc[static_cast<uint64_t>(row) * base_descriptors +
                              descriptor_id];
    const uint32_t start = static_cast<uint32_t>(descriptor);
    const uint32_t count = static_cast<uint32_t>(descriptor >> 32);
    valid = segment_candidate_descriptor_valid(
        start, count, row, descriptor_id % kSegmentCandidateDescriptorWarps,
        micro_records, pool_pages, physical_records, allocated_pages);
  } else if (has_spill) {
    const int spill_id = descriptor_id - base_descriptors;
    descriptor = spill_desc[static_cast<uint64_t>(row) * base_descriptors *
                                kSegmentCandidateSpillSlots +
                            spill_id];
    const uint32_t start = static_cast<uint32_t>(descriptor);
    const uint32_t count = static_cast<uint32_t>(descriptor >> 32);
    valid = segment_candidate_spill_descriptor_valid(
        start, count, micro_records, pool_pages, physical_records,
        allocated_pages);
  }
...
compact_topk_segmented_large_boundary_select(
    ...
    const int32_t* __restrict__ segment_idx,
    const uint64_t* __restrict__ segment_desc,
    const uint64_t* __restrict__ spill_desc, const int row, const int seed_n,
    const int desc_windows, const bool has_spill, const int micro_records,
    const int pool_pages, const int physical_records,
    const uint32_t allocated_pages, const int th, const int output_base,
    const int k_target, const int topk, int32_t* __restrict__ row_out,
    uint32_t* __restrict__ span_start, uint32_t* __restrict__ span_count,
    uint32_t* __restrict__ hist, uint32_t* __restrict__ desired,
    uint32_t* __restrict__ kfind, int* __restrict__ pivot_lt,
    int* __restrict__ write_lt, int* __restrict__ write_eq,
  ...
  const int num_descriptors =
      base_descriptors * (has_spill ? 1 + kSegmentCandidateSpillSlots : 1);

  if (tid == 0) {
    *desired = th == 0 ? 0u : (th_code & 0xff0000u);
    *kfind = static_cast<uint32_t>(k_target);
    *descriptors_valid = 1;
  }
  __syncthreads();
...
#pragma unroll
  for (int pass = 0; pass < 3; ++pass) {
    ...
    const uint32_t desired_base = *desired;
    compact_topk_paged_radix_hist_span(
        seed_val + seed_base, seed_idx + seed_base, seed_n, th, th_code,
        next_th_code, prefix_mask, desired_base, shift, hist);
    __syncthreads();
    for (int descriptor_id = 0; descriptor_id < num_descriptors;
         ++descriptor_id) {
      if (tid == 0) {
        compact_topk_segmented_descriptor_span(
            segment_desc, spill_desc, row, desc_windows, descriptor_id,
            has_spill, micro_records, pool_pages, physical_records,
            allocated_pages, span_start, span_count, descriptors_valid);
      }
      __syncthreads();
      compact_topk_paged_radix_hist_span(
          segment_val + *span_start, segment_idx + *span_start,
          static_cast<int>(*span_count), th, th_code, next_th_code, prefix_mask,
          desired_base, shift, hist);
      __syncthreads();
    }
    compact_find_radix_digit_warp0(hist, desired, kfind, desired_base, shift,
                                   tid);
  ...
  for (int descriptor_id = 0; descriptor_id < num_descriptors;
    ...
    if (tid == 0) {
      compact_topk_segmented_descriptor_span(
          segment_desc, spill_desc, row, desc_windows, descriptor_id, has_spill,
          micro_records, pool_pages, physical_records, allocated_pages,
    ...
    compact_topk_paged_count_pivot_lt_span(
        segment_val + *span_start, segment_idx + *span_start,
        static_cast<int>(*span_count), th, th_code, next_th_code, pivot,
        pivot_lt);
    __syncthreads();
  }
  ...
  for (int descriptor_id = 0; descriptor_id < num_descriptors;
    ...
    compact_topk_paged_write_pivot_span(
        segment_val + *span_start, segment_idx + *span_start,
        static_cast<int>(*span_count), th, th_code, next_th_code, pivot,
        output_base, exact_pivot_lt, eq_take, topk, row_out, write_lt,
        write_eq);
    __syncthreads();
  }
...
__global__ void compact_topk_min_thr_segmented_large_boundary_litetopk_kernel(
    const CandidateValue* __restrict__ seed_val,
    const int32_t* __restrict__ seed_idx, const int32_t* __restrict__ seed_cnt,
    const CandidateValue* __restrict__ segment_val,
    const int32_t* __restrict__ segment_idx,
  ...
  for (int tile_base = static_cast<int>(blockIdx.x) * kThreads;
    ...
    const int candidate_row = tile_base + tid;
    bool is_large = false;
    if (candidate_row < rows && finalizer_status[candidate_row] == 0) {
      ...
      is_large = tag < 0 && meta_th == th && th >= 0 && th < num_buckets &&
                 ...
                 spill_flag >= 0 && raw_pool_head >= 0 &&
                 raw_pool_head <= pool_pages && raw_n >= topk &&
                 raw_n <= logical_cap && meta_lt >= 0 && meta_lt < topk &&
    ...
    for (int task = 0; task < num_tasks; ++task) {
      ...
      if (tid == 0) {
        ...
        selected_allocated_pages = static_cast<uint32_t>(segment_pool_head[0]);
      }
      __syncthreads();
      int32_t* const row_out = out_idx + static_cast<uint64_t>(row) * topk;
      compact_topk_segmented_large_boundary_select(
          seed_val, seed_idx, segment_val, segment_idx, segment_desc,
          spill_desc, row, selected_seed_n, desc_windows, selected_has_spill,
          micro_records, pool_pages, physical_records, selected_allocated_pages,
          selected_th, selected_output_base, selected_k_target, topk, row_out,
          &span_start, &span_count, hist, &desired, &kfind, &pivot_lt,
          &write_lt, &write_eq, &descriptors_valid);
      dsa_litetopk_accumulate_inplace_votes(row_out, topk, tid, kThreads, votes,
                                            votes_len, row, vote_row_stride);
      __syncthreads();
    }
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> mqa_logits_dsa_litetopk(
  ...
  kernel<<<grid, SPEC_THREADS + MATH_THREADS, smem, stream>>>(
      ...
      (uint32_t)cand_cap, tm_q, tm_kv, tm_ks, tm_w, tm_q, nullptr, nullptr,
      nullptr, nullptr, nullptr, 0u, nullptr, nullptr, nullptr, nullptr,
      nullptr, 0u, 0u, nullptr, nullptr, nullptr, nullptr, 0u);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
...
static void mqa_logits_dsa_static_hot_litetopk_impl_t(
    ...
    bool exact_gate, std::optional<torch::Tensor> dump_slab = std::nullopt,
  ...
  if (raw_dump) {
    TORCH_CHECK(no_hist && !exact_gate,
                "raw-dump emit requires the plain no-hist writer");
    TORCH_CHECK(dump_counts.has_value(),
                "raw-dump emit needs both the region slab and the counts slab");
    const auto& slab = *dump_slab;
    const auto& counts = *dump_counts;
    TORCH_CHECK(
        slab.is_cuda() && counts.is_cuda() && slab.device() == q.device() &&
            counts.device() == q.device() && slab.is_contiguous() &&
            counts.is_contiguous() && slab.scalar_type() == torch::kInt &&
            counts.scalar_type() == torch::kInt,
        "dump slabs must be contiguous int32 CUDA tensors");
    const int64_t region_words =
        static_cast<int64_t>(kImplBlockQ) * dsa_litetopk::kEmitLaneSlots * 32;
  ...
  auto kernel =
      ...
      : no_hist ? &dsa_litetopk::sm100_dsa_litetopk<
                      kImplHeads, HEAD_DIM, kImplBlockQ, BLOCK_KV, NUM_Q_STAGES,
                      NUM_KV_STAGES, NUM_SMS, SPEC_THREADS, MATH_THREADS,
                      MATH_THREADS / 128, false, true, true, false, false,
                      false, true>
                : &dsa_litetopk::sm100_dsa_litetopk<
                      kImplHeads, HEAD_DIM, kImplBlockQ, BLOCK_KV, NUM_Q_STAGES,
                      NUM_KV_STAGES, NUM_SMS, SPEC_THREADS, MATH_THREADS,
  ...
  kernel<<<grid, SPEC_THREADS + MATH_THREADS, smem, stream>>>(
      ...
      tm_kv, tm_ks, tm_w, tm_q, nullptr, nullptr, nullptr, nullptr, nullptr, 0u,
      nullptr, nullptr, nullptr, nullptr, nullptr, 0u, 0u, nullptr, nullptr,
      raw_dump ? reinterpret_cast<uint32_t*>(dump_slab->data_ptr<int>())
               : nullptr,
      raw_dump ? reinterpret_cast<uint32_t*>(dump_counts->data_ptr<int>())
               : nullptr,
...
void mqa_logits_dsa_static_hot_litetopk_(
  ...
  mqa_logits_dsa_static_hot_litetopk_impl(q, kv, kv_scales, weights, cu_start,
                                          cu_end, origin, inv_delta, th_bucket,
...
void mqa_logits_dsa_static_hot_nohist_paged_litetopk_(
  ...
  TORCH_CHECK(
      q.size(1) == NUM_HEADS && q.size(2) == HEAD_DIM && kv.size(1) == HEAD_DIM,
...
void mqa_logits_dsa_static_hot_nohist_segmented_litetopk_(
    ...
    torch::Tensor weights, torch::Tensor cu_start, torch::Tensor cu_end,
    torch::Tensor origin, torch::Tensor inv_delta, torch::Tensor th_bucket,
    torch::Tensor seed_val, torch::Tensor seed_idx, torch::Tensor seed_cnt,
    torch::Tensor total_cnt, torch::Tensor bcount, torch::Tensor segment_val,
    torch::Tensor segment_idx, torch::Tensor segment_desc,
    torch::Tensor spill_desc, torch::Tensor row_spill,
    torch::Tensor segment_pool_head, torch::Tensor writer_status,
    int64_t num_buckets64, int64_t topk64) {
  const SegmentCandidateShape shape = check_segment_candidate_layout(
      seed_val, seed_idx, segment_val, segment_idx, segment_desc);
  check_segment_spill_layout(spill_desc, row_spill, segment_desc, shape);
  TORCH_CHECK(q.is_cuda() && kv.is_cuda() && kv_scales.is_cuda() &&
                  weights.is_cuda() && cu_start.is_cuda() && cu_end.is_cuda() &&
                  origin.is_cuda() && inv_delta.is_cuda() &&
  ...
  TORCH_CHECK(
      q.device() == kv.device() && q.device() == kv_scales.device() &&
          ...
          q.device() == cu_end.device() && q.device() == origin.device() &&
          q.device() == inv_delta.device() &&
          q.device() == th_bucket.device() && q.device() == seed_val.device() &&
          q.device() == seed_cnt.device() && q.device() == total_cnt.device() &&
          q.device() == bcount.device() &&
  ...
  TORCH_CHECK(q.is_contiguous() && kv.is_contiguous() &&
                  ...
                  segment_pool_head.is_contiguous() &&
                  writer_status.is_contiguous(),
  ...
  TORCH_CHECK(seq_len_kv <= (1 << dsa_litetopk::kCandidateIndexBits),
              "packed candidates support at most 1M KV positions");
  const int required_windows = (seq_len_kv + kSegmentCandidatePage * 16 - 1) /
                               (kSegmentCandidatePage * 16);
  TORCH_CHECK(shape.desc_windows == required_windows,
              "segment_desc W must equal ceil(S/65536)");
  TORCH_CHECK(weights.dim() == 2 && weights.size(0) == seq_len &&
                  weights.size(1) == NUM_HEADS && kv_scales.dim() == 1 &&
                  cu_start.dim() == 1 && cu_start.numel() == seq_len &&
                  cu_end.dim() == 1 && cu_end.numel() == seq_len,
              "weights/ranges must match Q and kv_scales must be 1-D");
  TORCH_CHECK(
      origin.dim() == 1 && origin.numel() == seq_len && inv_delta.dim() == 1 &&
          inv_delta.numel() == seq_len && th_bucket.dim() == 1 &&
          th_bucket.numel() == seq_len && seed_cnt.dim() == 1 &&
          seed_cnt.numel() == seq_len && total_cnt.dim() == 1 &&
          total_cnt.numel() == seq_len,
      "origin/inv_delta/threshold/seed_cnt/total_cnt must have Q elements");
  TORCH_CHECK(bcount.dim() == 2 && bcount.size(0) == seq_len &&
                  bcount.size(1) == num_buckets64,
              "bcount must be [Q,num_buckets]");
  TORCH_CHECK(segment_pool_head.numel() == 1 && writer_status.numel() == 1,
              "segment_pool_head/writer_status must be scalar int32 tensors");
  TORCH_CHECK(num_buckets64 >= 3 && num_buckets64 <= 256 && topk64 >= 1 &&
                  topk64 <= seq_len_kv,
  ...
  C10_CUDA_CHECK(cudaMemsetAsync(
      ...
      static_cast<size_t>(segment_desc.numel()) * sizeof(int64_t), stream));
  C10_CUDA_CHECK(cudaMemsetAsync(
      spill_desc.data_ptr<int64_t>(), 0,
      static_cast<size_t>(spill_desc.numel()) * sizeof(int64_t), stream));
  C10_CUDA_CHECK(cudaMemsetAsync(
      row_spill.data_ptr<int32_t>(), 0,
      static_cast<size_t>(row_spill.numel()) * sizeof(int32_t), stream));
  C10_CUDA_CHECK(cudaMemsetAsync(segment_pool_head.data_ptr<int32_t>(), 0,
                                 sizeof(int32_t), stream));
  C10_CUDA_CHECK(cudaMemsetAsync(writer_status.data_ptr<int32_t>(), 0,
                                 sizeof(int32_t), stream));
  ...
  kernel<<<grid, SPEC_THREADS + MATH_THREADS, smem, stream>>>(
      ...
      th_bucket.data_ptr<int32_t>(), bcount.data_ptr<int32_t>(),
      static_cast<uint32_t>(num_buckets), static_cast<uint32_t>(topk), 1u, 1u,
      0u, 0ULL, 0u, candidate_data_ptr(seed_val), seed_idx.data_ptr<int32_t>(),
      total_cnt.data_ptr<int32_t>(),
      static_cast<uint32_t>(kPagedCandidateInline), tm_q, tm_kv, tm_ks, tm_w,
      tm_q, nullptr, nullptr, nullptr, nullptr, nullptr, 0u,
      candidate_data_ptr(segment_val), segment_idx.data_ptr<int32_t>(),
      reinterpret_cast<uint64_t*>(segment_desc.data_ptr<int64_t>()),
      reinterpret_cast<uint32_t*>(segment_pool_head.data_ptr<int32_t>()),
...
void mqa_logits_dsa_static_hot_exactgate_nohist_litetopk_(
  ...
  mqa_logits_dsa_static_hot_litetopk_impl(
      q, kv, kv_scales, weights, cu_start, cu_end, origin, inv_delta, th_bucket,
      cand_val, cand_idx, cand_cnt, bcount, num_buckets64, topk64, true, true);
}
...
void h2048_safe_topk_out_litetopk_(torch::Tensor cand_val,
  ...
  const int64_t min_cap64 =
      std::max<int64_t>(16384, out_idx.dim() == 2 ? 32 * out_idx.size(1)
                                                  : h2048_safe_topk::kMinCap);
...
void compact_static_hot_candidates_litetopk_(
  ...
  TORCH_CHECK(cand_val.dim() == 2 && cand_idx.sizes() == cand_val.sizes(),
              "cand_val/cand_idx must be [R,cand_cap]");
  TORCH_CHECK(cand_val.size(0) <= std::numeric_limits<int>::max() &&
  ...
  c10::cuda::CUDAGuard device_guard(cand_val.device());
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream();
  compact_static_hot_candidates_litetopk_kernel<<<rows, 256, 0, stream>>>(
      candidate_data_ptr(cand_val), cand_idx.data_ptr<int32_t>(),
      cand_cnt.data_ptr<int32_t>(), th_bucket.data_ptr<int32_t>(),
      boundary_meta.data_ptr<int32_t>(), status.data_ptr<int32_t>(), rows,
      cand_cap, num_buckets, topk);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void finalize_static_hot_meta_mapped_paged_litetopk_(
    torch::Tensor inline_val, torch::Tensor inline_idx,
    torch::Tensor overflow_val, torch::Tensor overflow_idx,
    torch::Tensor page_table, torch::Tensor cand_cnt, torch::Tensor th_bucket,
    torch::Tensor boundary_meta, torch::Tensor status, torch::Tensor index_map,
    int64_t logical_cap64, int64_t num_buckets64, int64_t topk64) {
  const PagedCandidateShape shape =
      check_paged_candidate_layout(inline_val, inline_idx, overflow_val,
                                   overflow_idx, page_table, logical_cap64);
  TORCH_CHECK(cand_cnt.is_cuda() && th_bucket.is_cuda() &&
  ...
  TORCH_CHECK(cand_cnt.is_contiguous() && th_bucket.is_contiguous() &&
                  boundary_meta.is_contiguous() && status.is_contiguous() &&
  ...
  TORCH_CHECK(boundary_meta.dim() == 2 && boundary_meta.size(0) == shape.rows &&
  ...
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream();
  finalize_static_hot_meta_mapped_paged_litetopk_kernel<<<shape.rows, 256, 0,
                                                          stream>>>(
      candidate_data_ptr(inline_val), inline_idx.data_ptr<int32_t>(),
      candidate_data_ptr(overflow_val), overflow_idx.data_ptr<int32_t>(),
      page_table.data_ptr<int32_t>(), cand_cnt.data_ptr<int32_t>(),
      th_bucket.data_ptr<int32_t>(), boundary_meta.data_ptr<int32_t>(),
      status.data_ptr<int32_t>(), index_map.data_ptr<int32_t>(), index_map_size,
      shape.rows, shape.overflow_pages, shape.table_pages_per_row,
      shape.logical_cap, num_buckets, topk);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}

void finalize_static_hot_meta_mapped_segmented_litetopk_(
    torch::Tensor seed_val, torch::Tensor seed_idx, torch::Tensor seed_cnt,
    torch::Tensor segment_val, torch::Tensor segment_idx,
    torch::Tensor segment_desc, torch::Tensor spill_desc,
    torch::Tensor row_spill, torch::Tensor segment_pool_head,
    torch::Tensor writer_status, torch::Tensor total_cnt,
    torch::Tensor th_bucket, torch::Tensor boundary_meta, torch::Tensor status,
    torch::Tensor index_map, int64_t logical_cap64, int64_t num_buckets64,
    int64_t topk64) {
  const SegmentCandidateShape shape = check_segment_candidate_layout(
      seed_val, seed_idx, segment_val, segment_idx, segment_desc);
  check_segment_spill_layout(spill_desc, row_spill, segment_desc, shape);
  TORCH_CHECK(seed_cnt.is_cuda() && segment_pool_head.is_cuda() &&
                  writer_status.is_cuda() && total_cnt.is_cuda() &&
                  th_bucket.is_cuda() && boundary_meta.is_cuda() &&
                  status.is_cuda() && index_map.is_cuda(),
              "segmented finalizer metadata must be CUDA");
  TORCH_CHECK(seed_val.device() == seed_cnt.device() &&
  ...
  TORCH_CHECK(seed_cnt.is_contiguous() && segment_pool_head.is_contiguous() &&
  ...
  TORCH_CHECK(seed_cnt.scalar_type() == torch::kInt &&
                  segment_pool_head.scalar_type() == torch::kInt &&
                  writer_status.scalar_type() == torch::kInt &&
                  total_cnt.scalar_type() == torch::kInt &&
                  th_bucket.scalar_type() == torch::kInt &&
  ...
  TORCH_CHECK(logical_cap64 >= 1 &&
                  ...
                  num_buckets64 >= 3 && num_buckets64 <= 256 && topk64 >= 1 &&
                  topk64 <= logical_cap64,
              "invalid segmented logical_cap/num_buckets/topk");
  TORCH_CHECK(seed_cnt.dim() == 1 && seed_cnt.numel() == shape.rows &&
  ...
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream();
  finalize_static_hot_meta_mapped_segmented_litetopk_kernel<<<shape.rows, 256,
                                                              0, stream>>>(
      ...
      reinterpret_cast<const uint64_t*>(segment_desc.data_ptr<int64_t>()),
      reinterpret_cast<const uint64_t*>(spill_desc.data_ptr<int64_t>()),
      row_spill.data_ptr<int32_t>(), segment_pool_head.data_ptr<int32_t>(),
      writer_status.data_ptr<int32_t>(), total_cnt.data_ptr<int32_t>(),
      th_bucket.data_ptr<int32_t>(), boundary_meta.data_ptr<int32_t>(),
      status.data_ptr<int32_t>(), index_map.data_ptr<int32_t>(),
      static_cast<int>(index_map.numel()), shape.rows, shape.desc_windows,
      shape.micro_records, shape.pool_pages, shape.physical_records,
      static_cast<int>(logical_cap64), static_cast<int>(num_buckets64),
      static_cast<int>(topk64));
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void mqa_logits_dsa_online_fixed_litetopk_(
    ...
    torch::Tensor weights, torch::Tensor cu_start, torch::Tensor cu_end,
    torch::Tensor cand_val, torch::Tensor cand_idx, torch::Tensor cand_cnt,
    torch::Tensor bcount, torch::Tensor th_bucket, int64_t topk64) {
  TORCH_CHECK(q.is_cuda() && kv.is_cuda() && kv_scales.is_cuda() &&
                  weights.is_cuda() && cu_start.is_cuda() && cu_end.is_cuda() &&
                  cand_val.is_cuda() && cand_idx.is_cuda() &&
                  cand_cnt.is_cuda() && bcount.is_cuda() && th_bucket.is_cuda(),
              "all tensors must be CUDA");
  TORCH_CHECK(
      q.device() == kv.device() && q.device() == kv_scales.device() &&
          q.device() == weights.device() && q.device() == cu_start.device() &&
  ...
  online_fixed_init_litetopk_kernel<<<init_blocks, kInitThreads, 0, stream>>>(
      th_bucket.data_ptr<int32_t>(), cand_cnt.data_ptr<int32_t>(),
      bcount.data_ptr<int32_t>(), seq_len);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  ...
  auto kernel = &dsa_litetopk::sm100_dsa_litetopk<
      NUM_HEADS, HEAD_DIM, BLOCK_Q, BLOCK_KV, NUM_Q_STAGES, NUM_KV_STAGES,
      NUM_SMS, SPEC_THREADS, MATH_THREADS, MATH_THREADS / 128, true>;
  C10_CUDA_CHECK(
      cudaFuncSetAttribute(reinterpret_cast<void*>(kernel),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
  const int num_q_blocks = (seq_len + BLOCK_Q - 1) / BLOCK_Q;
  ...
  kernel<<<grid, SPEC_THREADS + MATH_THREADS, smem, stream>>>(
      static_cast<uint32_t>(seq_len), static_cast<uint32_t>(seq_len_kv),
      reinterpret_cast<uint32_t*>(cu_start.data_ptr<int>()),
      reinterpret_cast<uint32_t*>(cu_end.data_ptr<int>()), nullptr, nullptr,
      th_bucket.data_ptr<int32_t>(), bcount.data_ptr<int32_t>(),
      static_cast<uint32_t>(ONLINE_FIXED_BUCKETS), static_cast<uint32_t>(topk),
      1u, 1u, 0u, 0ULL, 0u, candidate_data_ptr(cand_val),
      cand_idx.data_ptr<int32_t>(), cand_cnt.data_ptr<int32_t>(),
      static_cast<uint32_t>(cand_cap), tm_q, tm_kv, tm_ks, tm_w, tm_q, nullptr,
      nullptr, nullptr, nullptr, nullptr, 0u, nullptr, nullptr, nullptr,
      nullptr, nullptr, 0u, 0u, nullptr, nullptr, nullptr, nullptr, 0u);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void map_topk_indices_and_accumulate_votes_litetopk_(
    ...
    torch::Tensor votes, int64_t vote_recent_rows64) {
  TORCH_CHECK(out_idx.is_cuda() && index_map.is_cuda() && status.is_cuda() &&
                  votes.is_cuda(),
  ...
  TORCH_CHECK(vote_recent_rows64 > 0 && vote_recent_rows64 <= out_idx.size(0),
  ...
  map_topk_indices_and_accumulate_votes_litetopk_kernel<<<blocks, kThreads, 0,
      ...
      static_cast<int>(vote_recent_rows64), nullptr, nullptr, nullptr, 0);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
...
void compact_topk_min_thr_paged_idx_out_litetopk(
    ...
    torch::Tensor overflow_val, torch::Tensor overflow_idx,
    torch::Tensor page_table, torch::Tensor cand_cnt, torch::Tensor th_bucket,
    torch::Tensor boundary_meta, int64_t logical_cap64, int64_t num_buckets64,
    int64_t topk64, torch::Tensor out_idx, torch::Tensor votes,
    int64_t vote_row_stride64) {
  const PagedCandidateShape shape =
      check_paged_candidate_layout(inline_val, inline_idx, overflow_val,
                                   overflow_idx, page_table, logical_cap64);
  ...
  TORCH_CHECK(
      votes.dim() == 1 && votes.numel() <= std::numeric_limits<int>::max(),
  ...
  c10::cuda::CUDAGuard device_guard(inline_val.device());
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream();
  compact_topk_min_thr_paged_idx_out_litetopk_kernel<<<shape.rows, 256, 0,
                                                       stream>>>(
      candidate_data_ptr(inline_val), inline_idx.data_ptr<int32_t>(),
      candidate_data_ptr(overflow_val), overflow_idx.data_ptr<int32_t>(),
      page_table.data_ptr<int32_t>(), cand_cnt.data_ptr<int32_t>(),
      th_bucket.data_ptr<int32_t>(), boundary_meta.data_ptr<int32_t>(),
      shape.rows, shape.overflow_pages, shape.table_pages_per_row,
      shape.logical_cap, num_buckets, topk, out_idx.data_ptr<int32_t>(),
      votes_len > 0 ? votes.data_ptr<int32_t>() : nullptr, votes_len,
      vote_row_stride);
  C10_CUDA_KERNEL_LAUNCH_CHECK();
  const int fallback_blocks = std::min((shape.rows + 255) / 256, NUM_SMS);
  compact_topk_min_thr_paged_large_boundary_litetopk_kernel<<<fallback_blocks,
                                                              256, 0, stream>>>(
      candidate_data_ptr(inline_val), inline_idx.data_ptr<int32_t>(),
      candidate_data_ptr(overflow_val), overflow_idx.data_ptr<int32_t>(),
      page_table.data_ptr<int32_t>(), cand_cnt.data_ptr<int32_t>(),
...
void compact_topk_min_thr_segmented_idx_out_litetopk(
    ...
    torch::Tensor segment_desc, torch::Tensor spill_desc,
    torch::Tensor row_spill, torch::Tensor segment_pool_head,
    torch::Tensor total_cnt, torch::Tensor th_bucket,
    torch::Tensor boundary_meta, torch::Tensor status, int64_t logical_cap64,
    int64_t num_buckets64, int64_t topk64, torch::Tensor out_idx,
    torch::Tensor votes, int64_t vote_row_stride64) {
  const SegmentCandidateShape shape = check_segment_candidate_layout(
      seed_val, seed_idx, segment_val, segment_idx, segment_desc);
  check_segment_spill_layout(spill_desc, row_spill, segment_desc, shape);
  TORCH_CHECK(seed_cnt.is_cuda() && total_cnt.is_cuda() &&
                  th_bucket.is_cuda() && boundary_meta.is_cuda() &&
                  status.is_cuda() && segment_pool_head.is_cuda() &&
                  out_idx.is_cuda() && votes.is_cuda(),
  ...
  TORCH_CHECK(seed_cnt.is_contiguous() && total_cnt.is_contiguous() &&
                  th_bucket.is_contiguous() && boundary_meta.is_contiguous() &&
  ...
  TORCH_CHECK(boundary_meta.dim() == 2 && boundary_meta.size(0) == shape.rows &&
                  boundary_meta.size(1) == num_buckets64,
              "boundary_meta must be [Q,num_buckets]");
  TORCH_CHECK(segment_pool_head.numel() == 1,
  ...
  c10::cuda::CUDAGuard device_guard(seed_val.device());
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream();
  compact_topk_min_thr_segmented_idx_out_litetopk_kernel<<<shape.rows, 256, 0,
                                                           stream>>>(
      candidate_data_ptr(seed_val), seed_idx.data_ptr<int32_t>(),
      seed_cnt.data_ptr<int32_t>(), candidate_data_ptr(segment_val),
      segment_idx.data_ptr<int32_t>(),
      ...
      boundary_meta.data_ptr<int32_t>(), status.data_ptr<int32_t>(), shape.rows,
...
void compact_topk_min_thr_inplace_idx_out_online_fixed_litetopk(
    torch::Tensor cand_val, torch::Tensor cand_idx, torch::Tensor cand_cnt,
    torch::Tensor th_bucket, torch::Tensor boundary_meta, int64_t k64,
    torch::Tensor out_idx, torch::Tensor votes, int64_t vote_row_stride64) {
  TORCH_CHECK(cand_val.is_cuda() && cand_idx.is_cuda() && cand_cnt.is_cuda() &&
                  th_bucket.is_cuda() && boundary_meta.is_cuda() &&
                  out_idx.is_cuda() && votes.is_cuda(),
              "tensors must be CUDA");
...
void mqa_logits_dsa_static_hot_nohist_dump_litetopk_(
    ...
    torch::Tensor cand_val, torch::Tensor cand_idx, torch::Tensor cand_cnt,
    torch::Tensor bcount, int64_t num_buckets64, int64_t topk64,
    torch::Tensor dump_slab, torch::Tensor dump_counts,
  ...
  if (nh == 64) {
    mqa_logits_dsa_static_hot_litetopk_impl_t<64, 2>(
        q, kv, kv_scales, weights, cu_start, cu_end, origin, inv_delta,
        th_bucket, cand_val, cand_idx, cand_cnt, bcount, num_buckets64, topk64,
        true, false, dump_slab, dump_counts, dump_max_windows);
  } else {
    mqa_logits_dsa_static_hot_litetopk_impl_t<32, 4>(
        q, kv, kv_scales, weights, cu_start, cu_end, origin, inv_delta,
        th_bucket, cand_val, cand_idx, cand_cnt, bcount, num_buckets64, topk64,
...
__global__ __launch_bounds__(32) void compact_dump_regions_litetopk_kernel(
    ...
    const int32_t* __restrict__ cu_start, const int32_t* __restrict__ cu_end,
    dsa_litetopk::CandidateValue* __restrict__ cand_val,
    int32_t* __restrict__ cand_idx, int32_t* __restrict__ cand_cnt,
    int32_t* __restrict__ status, const uint32_t seq_len,
    const uint32_t seq_len_kv, const uint32_t cand_cap,
...
#pragma unroll
  ...
  start = start / 4 * 4;
  const uint32_t nkv =
      end > start ? (end - start + BLOCK_KV - 1) / BLOCK_KV : 0u;
  const uint32_t windows = (nkv + dsa_litetopk::kEmitChunkBlocks - 1) /
                           dsa_litetopk::kEmitChunkBlocks;
  ...
  for (uint32_t w = 0; w < kScanWarps; ++w) {
    for (uint32_t win = 0; win < windows; ++win) {
      const uint64_t region =
          (static_cast<uint64_t>(qb) * kScanWarps + w) * max_windows + win;
      const uint32_t packed = dump_counts[region * 32u + lane];
      const uint32_t count = (packed >> (sub * 8)) & 0xffu;
      uint32_t prefix = count;
#pragma unroll
      for (int off = 1; off < 32; off <<= 1) {
        const uint32_t nsh = __shfl_up_sync(0xffffffffu, prefix, off);
        if (lane >= static_cast<uint32_t>(off)) prefix += nsh;
      }
      const uint32_t total = __shfl_sync(0xffffffffu, prefix, 31);
      if (total != 0u) {
        const uint32_t my_base = static_cast<uint32_t>(out) + prefix - count;
        const uint32_t kv_base =
            start + win * (dsa_litetopk::kEmitChunkBlocks * BLOCK_KV);
        const uint32_t max_count = __reduce_max_sync(0xffffffffu, count);
        const uint32_t* rbase = dump_slab +
                                region * (kBlockQ * kSlots * 32ull) +
                                (sub * kSlots) * 32u;
        const uint64_t row_out = static_cast<uint64_t>(row) * cand_cap;
        for (uint32_t slot = 0; slot < max_count; ++slot) {
          const bool live = slot < count;
          const uint32_t record = live ? __ldcs(rbase + slot * 32u + lane) : 0u;
          const uint32_t pos = my_base + slot;
          if (live && pos < cand_cap) {
            dsa_litetopk::store_candidate_record(
                &cand_val[row_out + pos], &cand_idx[row_out + pos], record >> 8,
                kv_base + (record & 0xffu) * BLOCK_KV + w * 32u + lane);
          }
        }
      }
  ...
  if (lane == 0) {
    if (static_cast<uint32_t>(out) > cand_cap) {
      dsa_litetopk::paged_candidate_set_status(
          status + row, dsa_litetopk::kPagedCandidateLogicalOverflow);
    }
    cand_cnt[row] = min(out, static_cast<int>(cand_cap));
  }
}

void compact_dump_regions_litetopk_(
    torch::Tensor dump_slab, torch::Tensor dump_counts, torch::Tensor cu_start,
    torch::Tensor cu_end, torch::Tensor cand_val, torch::Tensor cand_idx,
    torch::Tensor cand_cnt, torch::Tensor status, int64_t seq_len_kv,
    int64_t max_windows, int64_t block_q) {
  TORCH_CHECK(dump_slab.is_cuda() && dump_counts.is_cuda() &&
  ...
  kernel<<<static_cast<unsigned>(seq_len), 32, 0, stream>>>(
      ...
      cu_start.data_ptr<int32_t>(), cu_end.data_ptr<int32_t>(),
      candidate_data_ptr(cand_val), cand_idx.data_ptr<int32_t>(),
      cand_cnt.data_ptr<int32_t>(), status.data_ptr<int32_t>(),
      static_cast<uint32_t>(seq_len), static_cast<uint32_t>(seq_len_kv),
      static_cast<uint32_t>(cand_cap), static_cast<uint32_t>(max_windows));
  C10_CUDA_KERNEL_LAUNCH_CHECK();
}
  ...
  auto launch = [&](auto heads_c, auto blockq_c) {
    ...
    auto pick = [&](auto tight_c) {
      ...
      return &dsa_litetopk::sm100_dsa_litetopk_fp4graft<
          kH, HEAD_DIM, kBQ, BLOCK_KV, GRAFT_Q_STAGES, NUM_KV_STAGES_FP4,
          NUM_SMS, SPEC_THREADS, MATH_THREADS, kT, MATH_THREADS / 128>;
    };
    auto kernel = tighten ? pick(std::true_type{}) : pick(std::false_type{});
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("mqa_logits_dsa_static_hot_nohist_fp4graft_litetopk_",
        ...
        pybind11::arg("topk"));
  m.def("mqa_logits_dsa_static_hot_nohist_dump_litetopk_",
        ...
        pybind11::arg("cand_cnt"), pybind11::arg("bcount"),
        pybind11::arg("num_buckets"), pybind11::arg("topk"),
        pybind11::arg("dump_slab"), pybind11::arg("dump_counts"),
        pybind11::arg("dump_max_windows"));
  m.def("compact_dump_regions_litetopk_", &compact_dump_regions_litetopk_,
        "Decode raw dump regions into the canonical candidate slab",
        pybind11::arg("dump_slab"), pybind11::arg("dump_counts"),
        pybind11::arg("cu_start"), pybind11::arg("cu_end"),
        pybind11::arg("cand_val"), pybind11::arg("cand_idx"),
        pybind11::arg("cand_cnt"), pybind11::arg("status"),
        pybind11::arg("seq_len_kv"), pybind11::arg("max_windows"),
  ...
  m.def("map_topk_indices_and_accumulate_votes_litetopk_",
        ...
        pybind11::arg("out_idx"), pybind11::arg("index_map"),
        pybind11::arg("status"), pybind11::arg("votes"),
