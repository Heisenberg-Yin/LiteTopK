...
// q        [ng, 128, 512] bf16   row r = query r/h_per_q in the group, head
// r%h_per_q kv       [s_kv, 1, 512] bf16   flat dequantised workspace indices
// [ng, 1, topk] int32   ascending union list, -1 invalid, topk % 64 == 0 pref
// [ng, G] int32         prefix bound per query (union-rank space) win_lo   [ng,
// G] int32         window start per query (union-rank space) win_hi   [ng, G]
// int32         window end   per query (union-rank space) counts   [ng] int32
// union length per group (block-loop bound) attn_sink[128] fp32            per
// PACKED ROW (row r wants sink of head r%h_per_q) out      [ng, 128, 512] bf16
// lse      [ng, 128] fp32
void litedsa_masked_mla_dsv4_bf16(TensorView q, TensorView kv,
                                  TensorView indices, TensorView pref,
                                  TensorView win_lo, TensorView win_hi,
                                  TensorView counts, double sm_scale,
                                  int64_t h_per_q, TensorView out,
                                  TensorView lse, TensorView attn_sink) {
  ...
  TVM_FFI_ICHECK(pref.numel() == (int64_t)s_q * G &&
                 win_lo.numel() == (int64_t)s_q * G &&
                 win_hi.numel() == (int64_t)s_q * G)
      << "pref/win_lo/win_hi must be [ng, 128/h_per_q]";
  TVM_FFI_ICHECK(attn_sink.numel() == 128)
      << "attn_sink must be [128] (per packed row)";
  TVM_FFI_ICHECK(out.numel() == (int64_t)s_q * 128 * 512)
      << "out must be [ng, 128, 512]";
  ...
  SparseAttnFwdParams params = {
      ...
      static_cast<int32_t*>(
          counts.data_ptr()),  // topk_length: union length per group
