...
__device__ __forceinline__ void dense_topk_visit_row(
        const float* row, int begin, int end, int stride1, Fn fn) {
    if (stride1 == 1) {
        ...
        for (int offset = threadIdx.x; offset < prefix;
             offset += blockDim.x) {
        ...
        for (int i = prefix + n4 * 4 + threadIdx.x; i < len;
             i += blockDim.x) {
            fn(base[i - prefix], i);
        }
    } else {
...
using DenseTopkFusedScan =
    cub::BlockScan<int, kDenseTopkThreads>;
...
__global__ __launch_bounds__(kDenseTopkThreads)
void dense_topk_litetopk_kernel(
    ...
    int32_t* row_out =
        out + static_cast<int64_t>(row_idx) * topk;
    ...
    for (int bin = threadIdx.x; bin < kDenseTopkFineBins;
         bin += blockDim.x) {
    ...
    dense_topk_visit_row(
        row, interior_begin, interior_end, stride1, [&](float value, int) {
            atomicAdd(
                work.histogram.bins +
                    dense_topk_first_half_bin(value),
                1);
        });
    __syncthreads();
...
#pragma unroll
    for (int round = 0;
         round < kDenseTopkFineBins / kDenseTopkThreads;
         ++round) {
        const int bin =
            round * kDenseTopkThreads + threadIdx.x;
        const int count = work.histogram.bins[bin];
        int exclusive = 0;
        int aggregate = 0;
        DenseTopkFusedScan(work.histogram.scan).ExclusiveSum(
            count, exclusive, aggregate);
        ...
        [&](float value, int index) {
            ...
            if (bin < threshold) {
                ...
                if (pos < kDenseTopkBoundaryCap) {
                    work.boundary.codes[pos] =
                        dense_topk_descending_float_code(value);
    ...
    if (boundary_count <= kDenseTopkBoundaryCap) {
        ...
        for (int i = threadIdx.x; i < boundary_count;
             i += blockDim.x) {
        ...
        __syncthreads();
        dense_topk_write_streaming_result(
            row_out, selected, topk, free_k, prefix_len, suffix_len,
            row_len);
        return;
    }
...
#pragma unroll
    for (int pass = 0; pass < 3; ++pass) {
        ...
        dense_topk_visit_row(
            ...
            [&](float value, int) {
                ...
                const int digit = static_cast<int>(
                    (code >> shifts[pass]) & digit_masks[pass]);
        ...
        if (threadIdx.x == 0) {
            ...
            const uint32_t digit_mask =
                static_cast<uint32_t>(digit_masks[pass])
                << shifts[pass];
            radix_prefix |=
                static_cast<uint32_t>(chosen) << shifts[pass];
        ...
        if (radix_boundary_count >= 0 &&
            ...
            dense_topk_visit_row(
                row, interior_begin, interior_end, stride1,
                [&](float value, int index) {
                    if (dense_topk_first_half_bin(value) !=
                        threshold) {
                        return;
                    }
                    const uint32_t code =
            ...
            __syncthreads();
            dense_topk_write_streaming_result(
                row_out, selected, topk, free_k, prefix_len,
                suffix_len, row_len);
            return;
        }
    }
    ...
    __syncthreads();
    dense_topk_write_streaming_result(
        row_out, selected, topk, free_k, prefix_len, suffix_len, row_len);
}

__global__ __launch_bounds__(kDenseTopkThreads)
void dense_hist_meta_litetopk_kernel(
        const float* __restrict__ logits,
        const int32_t* __restrict__ row_starts,
        const int32_t* __restrict__ row_ends,
        int32_t* __restrict__ threshold,
        int32_t* __restrict__ count_lt,
        int32_t* __restrict__ count_eq,
        int stride0,
        int stride1,
        int topk,
        int bins,
    ...
    dense_topk_visit_row(row, begin, end, stride1, [&](float value, int) {
        atomicAdd(
            hist +
                (dense_topk_first_half_bin(value) >> hist_shift),
            1);
    });
    __syncthreads();

    const int rounds = (bins + kDenseTopkThreads - 1) /
        kDenseTopkThreads;
...
__global__ __launch_bounds__(kDenseTopkThreads)
void dense_prehist_select_litetopk_kernel(
        const float* __restrict__ logits,
        const int32_t* __restrict__ row_starts,
        const int32_t* __restrict__ row_ends,
        const int32_t* __restrict__ threshold,
        const int32_t* __restrict__ expected_lt,
        const int32_t* __restrict__ expected_eq,
        int32_t* __restrict__ out,
        int stride0,
        int stride1,
        int topk,
    ...
    if (threadIdx.x == 0) {
        const bool count_contract =
            direct_count == expected_lt[row_idx] &&
            boundary_count == expected_eq[row_idx];
        const bool rank_contract =
            direct_count < topk &&
            direct_count + boundary_count >= topk;
        metadata_valid = count_contract && rank_contract;
    }
    __syncthreads();

    if (metadata_valid &&
        boundary_count <= kDenseTopkBoundaryCap) {
        const int base = direct_count;
        for (int i = threadIdx.x; i < boundary_count;
             i += blockDim.x) {
            const uint32_t key = dense_topk_descending_float_code(
                work.boundary.values[i]);
            int rank = 0;
            for (int j = 0; j < boundary_count; ++j) {
                const uint32_t other =
                    dense_topk_descending_float_code(
                        work.boundary.values[j]);
    ...
    if (threadIdx.x == 0) {
        radix_target =
            metadata_valid ? topk - direct_count : topk;
...
#pragma unroll
    for (int pass = 0; pass < 3; ++pass) {
        ...
        const uint32_t pass_mask = radix_mask;
        dense_topk_visit_row(
            row, begin, end, stride1, [&](float value, int) {
                if (metadata_valid &&
                    (dense_topk_first_half_bin(value) >> hist_shift) !=
                        coarse_threshold) {
                    return;
...
inline void dense_topk_check_inputs(
...
inline void dense_topk_litetopk_(
        ...
        int64_t stride1,
        int64_t topk,
        int64_t num_init_tokens,
        int64_t num_local_tokens) {
    dense_topk_check_inputs(
        logits, row_starts, row_ends, rows, stride0, stride1, topk);
    dense_topk_check_output(logits, out, rows, topk);
    TORCH_CHECK(
        num_init_tokens >= 0 && num_local_tokens >= 0 &&
    ...
    dense_topk_litetopk_kernel<<<
        static_cast<int>(rows), kDenseTopkThreads, 0, stream>>>(
        ...
        static_cast<int>(num_init_tokens),
...
inline void dense_hist_meta_litetopk_(
        ...
        torch::Tensor count_lt,
        torch::Tensor count_eq,
        int64_t rows,
        int64_t stride0,
        int64_t stride1,
        int64_t topk,
    ...
    dense_topk_check_inputs(
        logits, row_starts, row_ends, rows, stride0, stride1, topk);
    dense_topk_check_metadata(
        logits, threshold, count_lt, count_eq, rows);
    TORCH_CHECK(
        bins == 512 || bins == 1024 || bins == 2048 || bins == 4096,
    ...
    const cudaStream_t stream =
        c10::cuda::getCurrentCUDAStream();
    dense_hist_meta_litetopk_kernel<<<
        static_cast<int>(rows), kDenseTopkThreads, 0, stream>>>(
        logits.data_ptr<float>(),
        row_starts.data_ptr<int32_t>(),
        row_ends.data_ptr<int32_t>(),
        threshold.data_ptr<int32_t>(),
        count_lt.data_ptr<int32_t>(),
        count_eq.data_ptr<int32_t>(),
        static_cast<int>(stride0),
        static_cast<int>(stride1),
        static_cast<int>(topk),
...
inline void dense_prehist_select_litetopk_(
    ...
    const cudaStream_t stream =
        c10::cuda::getCurrentCUDAStream();
    dense_prehist_select_litetopk_kernel<<<
        static_cast<int>(rows), kDenseTopkThreads, 0, stream>>>(
        logits.data_ptr<float>(),
        row_starts.data_ptr<int32_t>(),
        row_ends.data_ptr<int32_t>(),
