#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""B200 (SM100) DSA speedup SWEEP over (seq_len x chunk) on the REAL chunked
GLM-5 DSA caches (glm5_{tag}_realtext_chunk{N}.safetensors), where the query
batch Q == chunk is REAL prefill-chunk data (NOT tiled from Q=64).
...
(deep_gemm's official sm100_fp8_mqa_logits cannot run GLM-5's head_dim=128 /
num_heads=32 shape: it static-asserts kNumWeightsInReg=52 <= num_heads, i.e.
requires >=52 heads, per SGLang #19529. So the dense baseline is our own kernel.)

Run inside the `simtopk` container:
  PYTHONPATH=/opt/venvs/deepgemm/lib/python3.12/site-packages \
  DSA_CACHE_DIR=/data/dsa_caches \
  /usr/bin/python3.12 sweep_b200_real_chunk.py
"""
...
def main():
    ...
    for tag in TAGS:
        ...
        for chunk in CHUNKS:
            ...
            try:
                ...
                print(f"[{tag} chunk={chunk:>4}] Q={Q} S={S} DSA={tDSA:.3f}ms LiteTopK={tO:.3f}ms "
    ...
    jpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "figure_b200_realchunk.json")
    with open(jpath, "w") as f:
        json.dump(results, f, indent=2)
    print("\n===== DSA E2E (B200 SM100, REAL chunked GLM-5, Q=chunk, K=%d) =====" % K)
    print(f"{'seq':>5} {'chunk':>6} {'Q':>6} {'DSA(ms)':>9} {'LiteTopK(ms)':>12} {'speedup':>8} {'recall':>8}")
    for tag in TAGS:
        for chunk in CHUNKS:
            c = results[tag].get(str(chunk), {})
            if "DSA" in c:
                print(f"{tag:>5} {chunk:>6} {c['Q']:>6} {c['DSA']:>9.3f} {c['LiteTopK']:>12.3f} {c['speedup']:>7.2f}x {c['recall']:>7.2f}%")
    print("saved:", jpath)
