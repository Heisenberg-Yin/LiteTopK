...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> mqa_logits_dsa_marsco(
    ...
    const int topk_i = static_cast<int>(topk64);
#ifdef DENSE_WRITE
    // Dense build stores every logit at its absolute column: cap must be S.
    (void)cand_cap64;
    const int cand_cap = seq_len_kv;
#else
    // Sparse build: honor a caller-provided cap in [topk, S] so the candidate
    // buffers need not be [Q, S] (e.g. vLLM integration uses cap=64K instead of
    // 34GB-wide buffers at S=1M). Out-of-range values (the benchmark scripts
    // pass 1 or S) keep the legacy full-width behaviour. Kernel writes beyond
    // the cap are dropped (guarded) and the selects clamp cnt to the cap.
    const int cand_cap = (cand_cap64 >= topk_i && cand_cap64 < seq_len_kv)
