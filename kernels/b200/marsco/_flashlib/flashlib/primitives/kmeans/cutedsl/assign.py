...
The kernel ranks the shifted score ``c_sq - 2·cross`` (argmin matches
true ``||x − c||²``); ``x_sq`` is never loaded -- callers should NOT
precompute it. A legacy ``x_sq`` keyword is accepted for one release
cycle and silently dropped.
...
* In-register fused epilogue per centroid tile: the WGMMA accumulator
  ``(BM, BN_centroid)`` is converted to fp32 in registers, the shifted
  Euclidean score ``c_sq − 2·cross`` (no ``x_sq`` term, no clamp) is
  computed in registers, and a per-row running argmin is updated in
  registers. Across all centroid tiles, only ``(BM,)`` int32
  best-indices are written to GMEM per CTA.
...
def _try_init_cutedsl() -> bool:
    ...
    try:
        ...
        from flashlib.primitives.kmeans.cutedsl.assign_kernel import HopperFlashKmeansAssign
...
def cutedsl_assign_euclid(
    ...
    x_sq: Optional[torch.Tensor] = None,        # legacy, ignored (x²-free kernel)
    ...
    *,
    ...
    The kernel ranks the shifted score ``c_sq - 2·cross``; ``x_sq`` is
    never loaded. The legacy ``x_sq`` kwarg is accepted for one release
    cycle and silently dropped.
    ...
    """
    del x_sq, x_bf16, cross_buf, c_sq_buf  # legacy kwargs; x²-free kernel never uses them.
