...
updates a per-row running ``argmin`` across centroid tiles. The
``||x||²`` term is constant per row and so doesn't affect argmin --
dropping it saves one HBM tensor (x_sq), one register-file pre-load,
one FMA per accumulator element, and the ``dist >= 0`` underflow
clamp. Only the final ``(BM,)`` int32 best-index vector is written to
GMEM.
...
* In-register epilogue per centroid tile: convert the WGMMA acc to fp32
  (it is already fp32 if ``acc_dtype=Float32``), compute the signed
  shifted score ``s = c_sq − 2·cross`` per (m, n) acc element (no
  ``x_sq`` term, no underflow clamp), then do a per-thread linear
  argmin across the BN_centroid axis. The
...
class HopperFlashKmeansAssign:
    ...
    * ``use_ws=True``: "FMHA-style" warp-specialised kernel — adds a
    ...
    def kernel(
        ...
        # ------------------------------------------------------------
        # x²-free scoring: rank ``s = c_sq[k] - 2*cross``. The ``-||x||²``
        # term is a per-row constant and so does not affect argmin -- we
        # avoid loading x_sq from HBM entirely.
        # ------------------------------------------------------------
        ...
        for c_tile_idx in cutlass.range(num_c_tiles, unroll=1):
            ...
            # ------------------------------------------------------
            # In-register epilogue: shifted score ``s = c_sq - 2·acc``;
            # update per-thread running argmin over this tile's N_per_thr
            # elements. No ``x_sq`` (drops a HBM read + an FMA), and no
            # ``dist >= 0`` clamp (the score can be negative; argmin
            # still picks the same centroid).
            # ------------------------------------------------------
    ...
    def kernel_ws(
        ...
        if warp_group_idx >= self.num_load_warp_groups:
            ...
            # x²-free: no per-thread xs preload -- the rank-equivalent
            # shifted score ``c_sq - 2·cross`` drops the constant
            # ``||x[m]||²`` term that argmin doesn't need anyway.
            best_d = cute.make_rmem_tensor(cute.make_layout(M_per_thr), cutlass.Float32)
            best_i = cute.make_rmem_tensor(cute.make_layout(M_per_thr), cutlass.Int32)
            for i in cutlass.range_constexpr(M_per_thr):
                best_d[i] = cutlass.Float32(3.4e38)
