...
def euclid_assign_torch_native_chunked(x, centroids, chunk_size_N=32768, chunk_size_K=1024):
    """Torch reference assignment using the x²-free shifted score.

    Ranks ``s(n, k) = ||c[k]||² − 2·<x[n], c[k]>`` (argmin matches true
    ``||x − c||²`` because ``-||x||²`` is a per-row constant). Drops the
    ``x_sq`` precompute entirely.
...
def _euclid_iter_torch_naive(x, centroids, chunk_size_N=32768, chunk_size_K=1024):
    """One Lloyd iter using pure PyTorch (fallback when Triton is unavailable).

    Args:
        x: (B, N, D) input points
        centroids: (B, K, D) cluster centers
        chunk_size_N / chunk_size_K: chunking knobs to avoid OOM
