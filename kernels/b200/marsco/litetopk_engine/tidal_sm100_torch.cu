...
//
// Ops:
//   tidal_sm100::score_sparse(...)            -- correctness probe: bucket-gated
//       sparse scan only (validates the fp16 UMMA IP numerics on B200).
//   tidal_sm100::fused_ip_gqa_sparse_b200(...) -- full TidalDecode selection,
...
#include "sm100_tidal_marsco.cuh"
...
// Fused sample-prep (ported from dsa_marsco_v3's seed_prep_kernel, params-only
...
__global__ void tidal_seed_params_kernel(
...
// the TIDAL_KV_STAGES env of the loader.
#ifndef TIDAL_KV_STAGES
#define TIDAL_KV_STAGES 6
#endif
constexpr int NUM_KV_STAGES = TIDAL_KV_STAGES;
constexpr int NUM_SMS = 148;       // B200
#ifndef TIDAL_SPEC_THREADS
#define TIDAL_SPEC_THREADS 128
#endif
// 128: TMA + UMMA + 2 spare (refresh) warps。
// 64:  TMA + UMMA（C2：去掉 spare warp，refresh 内嵌到 math warp 的 gate 重读点；
//      384→320 线程把 launch_bounds 寄存器上限从 168 提到 204，缓解热路径 spill）。
constexpr int SPEC_THREADS = TIDAL_SPEC_THREADS;
...
static int compute_smem_bytes(int head_dim, int qn, int bm, bool out_fp32 = false) {
    ...
    const int smem_warpq = qn <= 8
        ? (bm / 32) * qn * ((int)sizeof(int32_t) + TIDAL_WARP_QUEUE_CAP * (cand_b + (int)sizeof(int32_t)))
        : 0;
...
#define TIDAL_DISPATCH_D(head_dim, ...)                                           \
    do {                                                                          \
...
static void launch_sm100_scan(
    ...
    TIDAL_DISPATCH_D(head_dim, {
        ...
        if (out_fp32) {
            ...
            if (bm == 256) {
                if (qn == 64) dispatch(&tidal_marsco::sm100_tidal_ip<64, DD, 256, 3, NUM_SMS, SPEC_THREADS, 256, float>, bp);
                else          dispatch(&tidal_marsco::sm100_tidal_ip<QN, DD, 256, 3, NUM_SMS, SPEC_THREADS, 256, float>, bp);
            } else {
                if (qn == 64) dispatch(&tidal_marsco::sm100_tidal_ip<64, DD, BM, STAGES, NUM_SMS, SPEC_THREADS, MATH_THREADS, float>, bp);
                else          dispatch(&tidal_marsco::sm100_tidal_ip<QN, DD, BM, STAGES, NUM_SMS, SPEC_THREADS, MATH_THREADS, float>, bp);
            }
        } else {
            auto* bp = reinterpret_cast<__half*>(buf_val.data_ptr<at::Half>());
            if (bm == 256) {
                if (qn == 64) dispatch(&tidal_marsco::sm100_tidal_ip<64, DD, 256, 3, NUM_SMS, SPEC_THREADS, 256, __half>, bp);
                else          dispatch(&tidal_marsco::sm100_tidal_ip<QN, DD, 256, 3, NUM_SMS, SPEC_THREADS, 256, __half>, bp);
            } else {
                if (qn == 64) dispatch(&tidal_marsco::sm100_tidal_ip<64, DD, BM, STAGES, NUM_SMS, SPEC_THREADS, MATH_THREADS, __half>, bp);
                else          dispatch(&tidal_marsco::sm100_tidal_ip<QN, DD, BM, STAGES, NUM_SMS, SPEC_THREADS, MATH_THREADS, __half>, bp);
            }
        }
    });
}
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> tidal_sm100_score_sparse(
...
at::Tensor tidal_sm100_dense_scores(const at::Tensor& x, const at::Tensor& kv_cont,
                                    int64_t num_ctas_x) {
    ...
    static const int kFlatQnD = [] {
        const char* e = getenv("TIDAL_FLAT_QN");
        return (e && atoi(e) == 64) ? 64 : 8;
    }();
    static const int kBmD = [] {
        const char* e = getenv("TIDAL_BM");
...
std::tuple<at::Tensor, at::Tensor> fused_ip_gqa_sparse_b200(
    ...
    // it stays opt-in (TIDAL_FLAT_QN=64) until the 64-row emit path gets a
    ...
    TORCH_CHECK(!out_fp32 || strided_sample,
                "out_fp32 requires sample_mode=1 (the tail-mode seed prefill writes fp16 candidates)");
    ...
    // QN-pad the query rows per group (rows [rows_valid, QN) stay zero);
    // flat batch needs no padding (every row is real).
    at::Tensor xpad;
    if (flat_batch) {
        xpad = x.contiguous();
    } else {
    ...
    if (flat_batch) {
        // raw scores (no neg): the fused prep kernel handles the x = -score
        // convention internally
        sample_scores = at::matmul(xpad, kv_tail.select(0, 0).transpose(0, 1));
        if (!strided_sample) sample_scores.neg_();
    } else {
        sample_scores = at::bmm(xpad.view({Hkv, QN, D}), kv_tail.transpose(1, 2)).neg_();
    }
    ...
    // topk_min/seed/offset/memset chain — see tidal_seed_params_kernel.
    if (strided_sample) {
        tidal_seed_params_kernel<<<(int)Rwork, 256, num_buckets * sizeof(int), stream>>>(
    ...
    // 2) seed top-k of the sample (min over x = top IP)
    ...
    if (!strided_sample) {
        launch_hopper_add_sample_idx_offset(
            sample_idx.data_ptr<int>(), (int)(Rwork * k), (int)(M - sample_size), stream);
    }

    // 3) bucket coordinates + threshold + seed candidates
    ...
    // tidal_seed_params_kernel — the scan histograms every candidate exactly
    ...
    if (scan_end > 0) {
        ...
        // no scan ran: tighten from the seed-only histogram on the host path
...
TORCH_LIBRARY(tidal_sm100, m) {
    ...
    m.def("fused_ip_gqa_sparse_b200(Tensor x, Tensor kv_cont, int k, int num_buckets, "
...
TORCH_LIBRARY_IMPL(tidal_sm100, CUDA, m) {
    m.impl("score_sparse", TORCH_FN(tidal_sm100_score_sparse));
    m.impl("fused_ip_gqa_sparse_b200", TORCH_FN(fused_ip_gqa_sparse_b200));
    m.impl("dense_scores", TORCH_FN(tidal_sm100_dense_scores));
}
