...
namespace tidal_marsco {

using namespace deep_gemm;
using namespace deep_gemm::sm90;
using namespace deep_gemm::sm100;

#ifndef TIDAL_REFRESH_STRIDE
#define TIDAL_REFRESH_STRIDE 16
#endif
...
#ifndef TIDAL_WARP_QUEUE
#define TIDAL_WARP_QUEUE 1
#endif
#ifndef TIDAL_WARP_QUEUE_CAP
#define TIDAL_WARP_QUEUE_CAP 32
#endif

#define TIDAL_ST_CAND_VAL(dst, v) __stcs(&(dst), (v))
#define TIDAL_ST_CAND_IDX(dst, v) __stcs(&(dst), (v))
...
__device__ __forceinline__ cand_t tidal_cand_cast(float x);
template <> __device__ __forceinline__ __half tidal_cand_cast<__half>(float x) { return __float2half(x); }
template <> __device__ __forceinline__ float tidal_cand_cast<float>(float x) { return x; }
...
void sm100_tidal_ip(const uint32_t M,                     // scan bound: rows [start_row, M) are scored
    ...
    // bs >= 32).
    constexpr bool kUseWarpQueue = TIDAL_WARP_QUEUE && (QN <= 8);
    ...
    auto warpq_idx = reinterpret_cast<int32_t*>(warpq_val + (kUseWarpQueue ? kNumMathWarps * QN * TIDAL_WARP_QUEUE_CAP : 0));
    ...
    auto coeff_o    = reinterpret_cast<float*>(warpq_idx + (kUseWarpQueue ? kNumMathWarps * QN * TIDAL_WARP_QUEUE_CAP : 0));
    ...
    if (is_tma_warp) {
        ...
        const uint32_t gate_stride = (refresh_every > 0 && refresh_every != 0x7fffffff)
                                         ? refresh_every : TIDAL_REFRESH_STRIDE;
        ...
        if constexpr (kUseWarpQueue) {
            DG_STATIC_ASSERT(TIDAL_WARP_QUEUE_CAP >= 32, "queue must hold one warp ballot");
        }
        ...
        const auto flush_warpq = [&](const uint32_t r, const uint32_t row) {
            ...
            const uint32_t qbase = slot * TIDAL_WARP_QUEUE_CAP;
            ...
            if (qn > 0) {
                ...
                for (int t2 = static_cast<int>(lane_idx); t2 < qn; t2 += 32) {
                    ...
                    if (w < static_cast<int>(buf_cap)) {
                        TIDAL_ST_CAND_VAL(buf_val[out_base + w], warpq_val[qbase + t2]);
                        TIDAL_ST_CAND_IDX(buf_idx[out_base + w], warpq_idx[qbase + t2]);
                    }
                }
            }
        ...
        for (uint32_t t = start_tile + blockIdx.y; t < num_kv_tiles_all; t += gridDim.y, ++consumed) {
            ...
            for (uint32_t sl = 0; sl < QN / 8; ++sl) {
                ...
                // tidal_sm100_torch.cu 的 debucket 步骤），不必在扫描/select 的每
                ...
                if constexpr (!kUseWarpQueue) {
                    ...
                    for (uint32_t r = 0; r < 8; ++r) {
                        ...
                        if ((pass_bits >> r) & 1u) {
                            ...
                            if (w < static_cast<int>(buf_cap)) {
                                TIDAL_ST_CAND_VAL(buf_val[static_cast<uint64_t>(row) * buf_cap + w], tidal_cand_cast<cand_t>(cand_reg[r]));
                                TIDAL_ST_CAND_IDX(buf_idx[static_cast<uint64_t>(row) * buf_cap + w], static_cast<int32_t>(kv_row));
                            }
                            if (refresh_every > 0)
                ...
                for (uint32_t r = 0; r < 8; ++r) {
                    ...
                    if (m != 0) {
                        ...
                        if constexpr (kUseWarpQueue) {
                            ...
                            if (qn + cnt > static_cast<int>(TIDAL_WARP_QUEUE_CAP)) {
                                int base = 0;
                                if (lane_idx == 0) base = atomicAdd(qcount + row, qn);
                                base = __shfl_sync(0xffffffffu, base, 0);
                                const uint64_t out_base = static_cast<uint64_t>(row) * buf_cap;
                            ...
                            if (g) {
                                ...
                                warpq_val[qbase + pos] = tidal_cand_cast<cand_t>(cand);
