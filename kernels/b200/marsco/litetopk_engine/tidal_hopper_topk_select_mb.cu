...
#ifndef FLASHTOPK_DIRECT_LT_MIN_CAP
#define FLASHTOPK_DIRECT_LT_MIN_CAP 0
#endif
...
inline FTLaunchParams ft_compute_params(int B, int N, int k, int sm_count) {
    ...
    uint32_t seg_stride = (uint32_t)FT_BLOCK_THREADS;   // 前缀：段紧挨着排
...
#if 0  // RETIRED: dense threshold kernels (fused_ip_dense / dense_bucket_fused only)
...
#endif  // dense hist/threshold kernels
...
// prefill compatibility — see sm100_tidal_marsco.cuh's epilogue comment).
...
__global__ void flashtopk_compact_thr_kernel(
    ...
    T* __restrict__ out_val,             // optional [R, K]：b<th 直接写最终输出
    int32_t* __restrict__ out_idx,       // optional [R, K]
    int skip_zero                        // !=0：跳过精确 0 值（dense-write 未写入的空槽）
) {
    using namespace ftns;
    int row = blockIdx.y;
    const T* data = buf + (size_t)row * M;
    ...
    const int32_t* sample_idx = sample_idx_in ? (sample_idx_in + (size_t)row * K) : nullptr;
    T*       ltv = lt_val ? (lt_val + (size_t)row * CAP) : nullptr;
    int32_t* lti = lt_idx ? (lt_idx + (size_t)row * CAP) : nullptr;
    ...
    // 输出处一次性转回（host 侧 debucket 步骤，见 tidal_sm100_torch.cu）。
    ...
    if constexpr (!BUCKET_SPACE) {
        ...
        inv_delta = ft_to_float<OT>(inv_delta_in[row]);
    }
    int th = th_in[row];
    ...
    //   （这块 [R,M] 只读一遍无复用）。条件：无 sample_idx 旁路（id 要么=i 要么=idx_data[i]，
    //   后者本身也是连续 int 数组，可同样 int4 向量读）+ 16B 对齐（M*sizeof(T)%16==0 且
    //   M%4==0 → val/idx 每行起始都对齐）。否则退化为标量逐元素读。
    ...
    if (can_vec) {
        ...
        __shared__ T s_v[STAGED ? 2 : 1][SCAP];
        __shared__ int32_t s_i[STAGED ? 2 : 1][SCAP];
        __shared__ int s_n[2];                     // [0]=lt, [1]=eq
        if (STAGED) {
            if (threadIdx.x == 0) { s_n[0] = 0; s_n[1] = 0; }
            __syncthreads();
        }
        const int4* data4 = reinterpret_cast<const int4*>(data);
        const int4* idx4 = idx_data ? reinterpret_cast<const int4*>(idx_data) : nullptr;
        int64_t M4 = (int64_t)M / VEC;
        int64_t lim4 = (limit + VEC - 1) / VEC; if (lim4 > M4) lim4 = M4;
        // STAGED 需全块同步 flush：所有线程必须走满相同轮数（含无工作的尾轮）。
        ...
        for (; base4 < lim4; base4 += stride) {
            ...
            const T* ve = reinterpret_cast<const T*>(&pk);
            // 先算本线程 VEC 个元素的命中掩码；仅当有命中且走 scatter-id 路径时才
            //   追加 idx 的 int4 读（idx 流量随命中密度而非全量走）。
            int hit_lt[VEC], hit_eq[VEC];
            int any_hit = 0;
            #pragma unroll
            for (int t = 0; t < VEC; ++t) {
                hit_lt[t] = 0; hit_eq[t] = 0;
                int64_t i = j4 * VEC + t;
                if (have && i < limit) {
                    float vf = ft_to_float<T>(ve[t]);
                    if (isfinite(vf) && !(skip_zero && vf == 0.0f)) {
                        // BUCKET_SPACE: vf 已是桶空间坐标 bq（扫描端存入），直接
                        // cast 即桶号——无需 origin/inv_delta 减法乘法（与扫描端
                        // 算 b_reg 的表达式天然一致，不变式无需额外重推）。
                        int braw = BUCKET_SPACE ? (int)vf : (int)((vf - origin) * inv_delta);
                        int b = braw < 0 ? 0 : (braw > NB - 1 ? NB - 1 : braw);
                        if (b < th) hit_lt[t] = 1; else if (b == th) hit_eq[t] = 1;
                        any_hit |= hit_lt[t] | hit_eq[t];
                    }
                }
            }
            int32_t ids[VEC];
            if (idx4 && any_hit) {
                #pragma unroll
                for (int u = 0; u < VEC / 4; ++u) {
                    int4 t4 = __ldcs(idx4 + j4 * (VEC / 4) + u);
                    ids[u * 4 + 0] = t4.x; ids[u * 4 + 1] = t4.y;
                    ids[u * 4 + 2] = t4.z; ids[u * 4 + 3] = t4.w;
                }
            }
            if constexpr (STAGED) {
                #pragma unroll
                for (int t = 0; t < VEC; ++t) {
                    if (hit_lt[t] | hit_eq[t]) {
                        int which = hit_eq[t];
                        int pos = atomicAdd(&s_n[which], 1);
                        s_v[which][pos] = ve[t];
                        s_i[which][pos] = idx4 ? ids[t] : (int32_t)(j4 * VEC + t);
                    }
                }
                __syncthreads();
                #pragma unroll
                for (int which = 0; which < 2; ++which) {
                    // 水位：下一轮最多再来 BLOCK*VEC 条；最后一轮无条件清空。
                    int n = s_n[which];
                    bool last = base4 + stride >= lim4;
                    if (n > 0 && (last || n > SCAP - BLOCK_THREADS_ * VEC)) {
                        __shared__ int s_base[2];
                        if (threadIdx.x == 0)
                            s_base[which] = atomicAdd(which ? (eq_cnt + row) : (lt_cnt + row), n);
                        __syncthreads();
                        int gbase = s_base[which];
                        T*       dv = which ? eqv : ltv;
                        int32_t* di = which ? eqi : lti;
                        for (int j = threadIdx.x; j < n; j += BLOCK_THREADS_) {
                            int w = gbase + j;
                            if (w < CAP) { dv[w] = s_v[which][j]; di[w] = s_i[which][j]; }
                        }
                        __syncthreads();
                        if (threadIdx.x == 0) s_n[which] = 0;
                    }
                    __syncthreads();
                }
            } else {
                #pragma unroll
                for (int t = 0; t < VEC; ++t) {
                    int is_lt = hit_lt[t], is_eq = hit_eq[t];
                    T v = T(); int32_t id = 0;
                    int64_t i = j4 * VEC + t;
                    if (is_lt || is_eq) {
                        v = ve[t];
    ...
    for (; base_i < limit; base_i += stride) {
        ...
        if (i < limit) {
            ...
            float vf = ft_to_float<T>(v);
            if (isfinite(vf) && !(skip_zero && vf == 0.0f)) {
                int braw = BUCKET_SPACE ? (int)vf : (int)((vf - origin) * inv_delta);  // 见上方 vec 路径注释
                int b = braw < 0 ? 0 : (braw > NB - 1 ? NB - 1 : braw);
                if (b < th) is_lt = 1;
...
template <typename T, int BLOCK_THREADS_>
__global__ void flashtopk_bucket_compact_thr_kernel(
    const T* __restrict__ bucket_val,
    const int32_t* __restrict__ bucket_idx,
    const int32_t* __restrict__ bcount,
    int NB, int BUCKET_CAP, int CAP, int K,
    const int32_t* __restrict__ th_in,
    T* __restrict__ lt_val,
    int32_t* __restrict__ lt_idx,
    int32_t* __restrict__ lt_cnt,
    T* __restrict__ eq_val,
    int32_t* __restrict__ eq_idx,
    int32_t* __restrict__ eq_cnt) {
    int row = blockIdx.y;
    int b = blockIdx.x;
    int th = th_in[row];
    if (b > th) return;
    int cnt = bcount[row * NB + b];
    if (cnt < 0) cnt = 0;
    int max_run = (NB - b) * BUCKET_CAP;
    if (cnt > max_run) cnt = max_run;
    const T* bv = bucket_val + ((size_t)row * NB + b) * BUCKET_CAP;
    const int32_t* bi = bucket_idx + ((size_t)row * NB + b) * BUCKET_CAP;
    T* outv = (b < th) ? (lt_val + (size_t)row * CAP) : (eq_val + (size_t)row * CAP);
    int32_t* outi = (b < th) ? (lt_idx + (size_t)row * CAP) : (eq_idx + (size_t)row * CAP);
    int32_t* outc = (b < th) ? (lt_cnt + row) : (eq_cnt + row);
    int lane = threadIdx.x & 31;
    unsigned lanemask = (1u << lane) - 1u;
    for (int i = threadIdx.x; i < cnt; i += BLOCK_THREADS_) {
        ft_warp_emit<T>(1, bv[i], bi[i], lane, lanemask, outc, outv, outi, CAP);
    }
}
...
// Hierarchical finalize for large 1M candidate buffers:
//   stage 1: split each row's select source into chunks, one block emits local top-k.
...
__global__ void flashtopk_boundary_radix_partial_kernel(
    ...
    const T* __restrict__ eq_val, const int32_t* __restrict__ eq_idx,
    const int32_t* __restrict__ eq_cnt,
    int CAP, int K, int chunks_per_row,
    ...
    int cnt_lt = lt_cnt[row]; if (cnt_lt < 0) cnt_lt = 0; if (cnt_lt > CAP) cnt_lt = CAP;
    int cnt_eq = eq_cnt[row]; if (cnt_eq < 0) cnt_eq = 0; if (cnt_eq > CAP) cnt_eq = CAP;
    int pslot = row * chunks_per_row + chunk;
    T* pv = part_val + (size_t)pslot * K;
    int32_t* pi = part_idx + (size_t)pslot * K;
    if (tidx == 0) part_cnt[pslot] = 0;
    ...
    int k_find;
    if (cnt_lt >= K) {
        data = ltd; idx_data = lti; n = cnt_lt; k_find = K;
    } else {
        data = eqd; idx_data = eqi; n = cnt_eq; k_find = K - cnt_lt;
    }

    int slice_begin = (int)(((int64_t)n * chunk) / chunks_per_row);
    int slice_end = (int)(((int64_t)n * (chunk + 1)) / chunks_per_row);
    int slice_n = slice_end - slice_begin;
    if (slice_n <= 0 || k_find <= 0) return;
    int take = k_find < K ? k_find : K;
    if (take > slice_n) take = slice_n;
    data += slice_begin;
    idx_data += slice_begin;
    if (tidx == 0) part_cnt[pslot] = take;

    if (slice_n <= take) {
        for (int i = tidx; i < slice_n; i += BLOCK_THREADS_) {
            pv[i] = data[i];
            pi[i] = idx_data[i];
        }
        return;
    ...
    if (tidx == 0) { s_desired = 0u; s_kfind = (uint32_t)take; }
    __syncthreads();
    const int num_passes = EncTraits<T>::kPasses;
    const int start_bit = EncTraits<T>::kBits - FT_RADIX_BITS;
    uint32_t desiredMask = 0u;
    ...
    __syncthreads();
    int cnt_lt2 = s_cnt_lt;
    int eq_take = take - cnt_lt2; if (eq_take < 0) eq_take = 0;
    ...
    for (int i = tidx; i < slice_n; i += BLOCK_THREADS_) {
        ...
        if (e < pivot) {
            int w = atomicAdd(&s_w_lt, 1);
            if (w < K) { pv[w] = vv; pi[w] = idx_data[i]; }
        } else if (e == pivot) {
            int o = atomicAdd(&s_w_eq, 1);
            if (o < eq_take) {
                int pos = cnt_lt2 + o;
                if (pos < K) { pv[pos] = vv; pi[pos] = idx_data[i]; }
            }
        }
    }
}
...
__global__ void flashtopk_boundary_radix_merge_kernel(
    ...
    const int total_slots = chunks_per_row * K;
    T* ov = out_val + (size_t)row * K;
    int32_t* oi = out_idx + (size_t)row * K;
    const T* pv = part_val + (size_t)row * chunks_per_row * K;
    const int32_t* pi = part_idx + (size_t)row * chunks_per_row * K;
    const int32_t* pc = part_cnt + row * chunks_per_row;

    __shared__ int s_total_valid;
    if (tidx == 0) s_total_valid = 0;
    __syncthreads();
    for (int c = tidx; c < chunks_per_row; c += BLOCK_THREADS_) {
        int n = pc[c]; if (n < 0) n = 0; if (n > K) n = K;
        atomicAdd(&s_total_valid, n);
    }
    __syncthreads();
    if (s_total_valid <= k_find) {
        __shared__ int s_w_all;
        if (tidx == 0) s_w_all = 0;
        __syncthreads();
        for (int i = tidx; i < total_slots; i += BLOCK_THREADS_) {
            int c = i / K;
            int off = i - c * K;
            int n = pc[c]; if (n < 0) n = 0; if (n > K) n = K;
            if (off < n) {
    ...
    for (int pass = 0; pass < num_passes; ++pass) {
        ...
        for (int i = tidx; i < total_slots; i += BLOCK_THREADS_) {
            ...
            int n = pc[c]; if (n < 0) n = 0; if (n > K) n = K;
            if (off < n) {
                uint32_t e = ft_enc_val<T>(pv[i]) ^ enc_xor;
                if ((e & desiredMask) == (desired & desiredMask))
                    atomicAdd(&ts.digit_counters[getDigit(e, current_bit)], 1u);
            }
        }
...
void flash_topk_select_thr_mb(const T* buf, const int32_t* buf_idx, const int32_t* sample_idx,
    ...
    // 大 k（无 direct-lt）走 STAGED 实例：smem 暂存 + 整块 flush，砍掉 per-warp 全局原子
    //   （ncu：long_scoreboard 59 cyc/issue 全在计数器原子 L2 往返上）。direct-lt 路径
    //   需要逐 warp 判 w<K 写最终输出，保持原 warp-emit 实例。
    constexpr int kVecT = (int)(sizeof(int4) / sizeof(T));
...
template <typename T>
void flash_topk_select_bucket(const T* bucket_val, const int32_t* bucket_idx,
                              const int32_t* bcount, int R, int NB, int BUCKET_CAP, int K,
                              const int32_t* th, int CAP,
                              T* cand_val, int32_t* cand_idx, int32_t* cand_cnt,
                              T* lt_val, int32_t* lt_idx, int32_t* lt_cnt,
                              T* out_val, int* out_idx, cudaStream_t stream,
                              uint32_t enc_xor = 0u) {
    ...
    if (R <= 0 || K <= 0) return;
    cudaMemsetAsync(lt_cnt, 0, sizeof(int32_t) * (size_t)R, stream);
    cudaMemsetAsync(cand_cnt, 0, sizeof(int32_t) * (size_t)R, stream);
    dim3 grid_c(NB, R);
...
#if 0  // RETIRED: dense threshold host template (no live caller)
...
#endif  // flash_topk_dense_threshold template
} // namespace
...
void launch_flash_topk_select_bucket_idx_fp16(
    ...
    flash_topk_select_bucket<__half>(
        bucket_val, bucket_idx, bcount, R, NB, BUCKET_CAP, K, th, CAP,
        cand_val, cand_idx, cand_cnt, lt_val, lt_idx, lt_cnt, out_val, out_idx, stream);
}
...
#if 0  // RETIRED: dense threshold launchers (no live caller)
// ---- DENSE 直方图+阈值 launch 包装 ------------------------------------------
// Threshold-only: bcount (already accumulated) -> th.  Used by the fused
// full-store+histogram path where the GEMM epilogue already built bcount.
...
void launch_flash_topk_dense_threshold_fp16(
        ...
        bool skip_bcount_zero) {
    if (coords_fp16)
        flash_topk_dense_threshold<__half, __half>(buf, R, M, K,
            reinterpret_cast<const __half*>(origin), reinterpret_cast<const __half*>(inv_delta),
            NB, bcount, th, stream, skip_bcount_zero);
    else
...
#endif  // dense threshold launchers
