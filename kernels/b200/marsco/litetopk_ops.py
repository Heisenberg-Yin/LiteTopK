"""JIT loader + Python API for the B200 (SM100) LiteTopK MS MARCO IP top-k kernel.

Builds one torch extension from `litetopk_select.cu` (FlashTopk select engine) +
`litetopk_sm100_torch.cu` (the SM100 UMMA/TMEM sparse-scan host binding) and exposes
the main entry `fused_ip_sparse_b200(...)` = `torch.ops.litetopk_sm100.fused_ip_sparse_b200`.
...
def fused_ip_sparse_b200(
    ...
    Pipeline: sample -> gate (bucket threshold) -> SM100 warp-specialized
