...
// ---- DENSE 直方图+阈值（替代 Triton 的 global 原子直方图）---------------------
// DENSE 模式 _flat 写全量 score 到 [R,M] 后，本 op 用 SMEM 直方图算每行阈值桶 th。
//   buf      : [R, M] 行优先连续，未写位置为 NaN（isfinite 跳过）
//   origin/inv_delta : [R]，float32 或与 buf 同 dtype（coords_* 标志）
//   bcount   : [R, NB] int32 工作区（内部清零）；th : [R] int32 输出
