...
def main():
    ...
    # Clear the ‖c‖² cache each iter so the corpus-side preprocessing is
    # re-paid inside the timed region, matching the two baselines (which
    # recompute csq inside their timed fn).
    def fn():
        _csq_cache.clear()
