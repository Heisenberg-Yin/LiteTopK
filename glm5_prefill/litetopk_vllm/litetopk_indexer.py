...
_DSA_DIR = os.environ.get("LITETOPK_DSA_DIR", "/opt/simtopk_repro/kernels/b200/dsa")
...
def _ext():
    ...
    if _EXT is None and not _FAILED:
        try:
            ...
            if KERNEL == "v3":
                name, src, bdir = ("litetopk_dsa_b200_v3", "dsa_marsco_v3.cu",
...
def stash_carry(hot_key, idx, S):
    ...
    by the container on the LAST official chunk before MIN_S). The boundary
    ours-band call then runs HOT: no cold start, no cap-floor spike, tighter
    thresholds than the cold prefix path. Stored compressed (voted hot
    columns, ~64KB/layer)."""
    ...
    _HOT_CARRY[(str(dev), hot_key)] = (hot, nv, None)
...
def try_chunk(q, k, k_scale, weights, ks, ke, out_idx, topk,
    ...
    try:
        ...
        if hot_here:
            ...
            _smp, slog = (k[:head], k_scale[:head]), None
            ks_scan = keh  # scan starts after the sampled prefix
        cap_eff = cap if cap is not None else CAP
        if HOTONLY and HOTSAMPLE > 0 and not hot_here:
            # any call the hot branch did NOT serve (cold start, or the
            # S=262144 boundary chunk that runs PREFIX mode where the carry
            # is ignored): it falls through to the exact probe/prefix path,
            # whose candidate counts are ~K*span/sample (5-16K at 262K) --
            # the 2xK hot cap would silently truncate = recall breach.
            # One-off flagship cap.
            cap_eff = max(cap_eff, CAP_CEIL)
        _repair_pack = None
